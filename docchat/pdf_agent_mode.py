"""
PDF Agent Mode - Sistema Multi-Agente RAG de Máxima Calidad
...
class PDFAgentMode:
    """
    PDF Agent Mode - Sistema Multi-Agente RAG de Máxima Calidad para Empresas.
    ...
    def __init__(
        ...
        if not config.openai_api_key and not config.anthropic_api_key:
            raise ValueError("OPENAI_API_KEY o ANTHROPIC_API_KEY requerida para PDF Agent Mode")
    ...
    def _get_llm_for_provider(self, provider: str = "openai"):
        ...
        if provider_lower in ["claude", "anthropic"]:
            ...
            if not api_key:
                print(f"⚠️ [PDF Agent Mode] ANTHROPIC_API_KEY no configurada, usando OpenAI como fallback")
            ...
            api_key = self.config.openai_api_key
            if not api_key:
                raise ValueError("OPENAI_API_KEY requerida para PDF Agent Mode")
    ...
    def process_documents(
        ...
        try:
            print(f"📄 [PDF Agent Mode] Procesando {len(new_files)} nuevos documentos...")
            ...
            if session["docs"]:
                ...
                print(f"✅ [PDF Agent Mode] Retriever actualizado: {len(session['docs'])} chunks")
            ...
            print(f"❌ [PDF Agent Mode] Error procesando documentos: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    async def process_query_async(
        ...
        try:
            ...
            print(f"⚠️ [PDF Agent Mode] Error agregando pasos de razonamiento: {e}")
        ...
        try:
            ...
            print(f"⚠️ [PDF Agent Mode] Error en Reinforcement Planning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [PDF Agent Mode] Error en Path-dependent Reasoning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [PDF Agent Mode] Error consultando MCP: {e}")
        ...
        try:
            ...
            for conn_id, connection in self.mcp_manager.connections.items():
                ...
                try:
                    ...
                    print(f"⚠️ [PDF Agent Mode] Error consultando MCP {connection.name}: {e}")
                    continue
            ...
            print(f"⚠️ [PDF Agent Mode] Error en consulta MCP: {e}")
            return None
...
RESPUESTA ESPECÍFICA A LA PREGUNTA DEL USUARIO (300-500 palabras):"""
        ...
        print(f"🔄 [PDF Agent Mode] Procesando {len(docs_by_source)} documentos en paralelo...")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"✅ [PDF Agent Mode] Análisis completado para: {Path(doc_name).name}")
                except Exception as e:
                    print(f"❌ [PDF Agent Mode] Error procesando {source_name}: {e}")
...
# Instancia global
_pdf_agent_mode_instance: Optional[PDFAgentMode] = None


def get_pdf_agent_mode(
    config: AppConfig,
    processor: DocumentProcessor,
    retriever_builder: RetrieverBuilder,
    context_manager: Optional[Any] = None
) -> PDFAgentMode:
    """Obtiene o crea la instancia global de PDF Agent Mode."""
    global _pdf_agent_mode_instance
    
    if _pdf_agent_mode_instance is None:
        _pdf_agent_mode_instance = PDFAgentMode(
            config=config,
            processor=processor,
    ...
    return _pdf_agent_mode_instance


def run_pdf_agent_mode(
    message: str,
    history: List[Tuple[str, str]],
    files: List[Any],
    session_id: str,
    ...
    """
    Función principal para ejecutar PDF Agent Mode.
    ...
    try:
        ...
        new_history, error, metadata = loop.run_until_complete(
            pdf_agent_mode.process_query_async(
                session_id=session_id,
                message=message,
                history=history,
                speed_mode=speed_mode,
