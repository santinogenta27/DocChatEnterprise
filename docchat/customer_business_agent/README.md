...
- `customer_business_agent_mode.py`: modo principal (`CustomerBusinessAgentMode`)
- `agents/customer_business_agent.py`: orquestador de ventas + soporte
...
config = load_config()
mode = CustomerBusinessAgentMode(config=config)
...
app = FastAPI()
mode = CustomerBusinessAgentMode(config=load_config())
