"""Assistance AI - Modo principal.

Agente unificado de asistencia 24/7 para todos los canales.
"""
...
from .agents.assistance_ai_agent import AssistanceAIAgent, AssistanceAIConfig
...
class AssistanceAIMode:
    """Modo principal Assistance AI."""
    ...
    def __init__(self, config: Optional[AppConfig] = None, llm: Optional[BaseLanguageModel] = None) -> None:
        ...
        if llm is not None:
            ...
            # SIEMPRE usar Groq para Assistance AI (como solicitado)
            groq_api_key = self.config.groq_api_key or os.getenv("GROQ_API_KEY")
            if not groq_api_key:
                raise ValueError("GROQ_API_KEY requerida para Assistance AI. Configura en .env: GROQ_API_KEY=tu-clave")
            
            try:
                from langchain_groq import ChatGroq
                # SIEMPRE usar Groq 3.3 70B para Assistance AI
                groq_llm = ChatGroq(
                ...
                self.llm = groq_llm
                print("✅ Assistance AI usando Groq (Llama 3.3 70B Versatile) - Velocidad <0.5 seg")
            except ImportError:
        ...
        if self.config.postgresql_enabled and self.config.postgresql_url:
            try:
                ...
                print("✅ Assistance AI usando PostgreSQL - Memoria de largo plazo activa")
            except Exception as e:
        ...
        self.agent = AssistanceAIAgent(
            ...
            config=AssistanceAIConfig(
                ...
                use_langgraph=True,  # SIEMPRE ACTIVADO - LangGraph Agent Enterprise
            ),
    ...
    def get_api_router(self) -> APIRouter:
        router = APIRouter(prefix="/business-ai", tags=["Assistance AI"])

        @router.post("/chat")
        async def chat(payload: Dict[str, Any]) -> Dict[str, Any]:
            """Endpoint de chat para Assistance AI Widget"""
            try:
    ...
    def get_gradio_interface(self):  # type: ignore[override]
        ...
        return gr.Interface(fn=_chat_fn, inputs=["text", "text"], outputs="text", title="Assistance AI")
