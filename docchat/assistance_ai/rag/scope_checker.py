...
class ScopeChecker:
    """Verifica si una pregunta está dentro del alcance de los documentos."""
    
    def __init__(self, llm: BaseLanguageModel, retriever: Optional[BaseRetriever] = None):
        """Inicializa el Scope Checker.
        
        Args:
            llm: Modelo de lenguaje para verificación (Groq/OpenAI)
    ...
    def check(self, question: str, k: int = 3) -> str:
        """Verifica si la pregunta está en el alcance de los documentos.
        
        Args:
            question: Pregunta del usuario
            k: Número de documentos a recuperar para verificación
        ...
        try:
            ...
            if hasattr(self.retriever, 'get_relevant_documents'):
                # Intentar con parámetro k si el retriever lo soporta
                try:
...
**Instrucciones:**
- Clasifica qué tan bien el contenido del documento responde a la pregunta del usuario.
- Responde SOLO con una de estas etiquetas: CAN_ANSWER, PARTIAL, NO_MATCH
- CAN_ANSWER: Los documentos proporcionan suficiente información para una respuesta completa
...
**Responde SOLO con: CAN_ANSWER, PARTIAL, o NO_MATCH**"""
            ...
            # Default a NO_MATCH si no es válido
            return "NO_MATCH"
            
        except Exception as e:
            print(f"⚠️ Error en scope checking: {e}")
            # En caso de error, permitir que continúe (mejor tener respuesta que bloquear)
            return "CAN_ANSWER"
