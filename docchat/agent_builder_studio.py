"""
Agent Builder Studio - Plataforma No-Code para Crear AI Agents Personalizados
...
class AgentTemplate(Enum):
    """Templates pre-construidos para diferentes casos de uso."""
    ...
    def _initialize_templates(self):
        ...
        self.templates = {
            AgentTemplate.CUSTOMER_SUPPORT: {
                ...
                "description": "Agente especializado en atención al cliente y resolución de problemas",
                "system_prompt": """Eres un agente de atención al cliente profesional y empático.
Tu objetivo es ayudar a los clientes de manera eficiente y amigable.
- Escucha activamente las preocupaciones del cliente
- Proporciona soluciones claras y paso a paso
...
- Mantén un tono amigable y útil""",
    ...
    def create_agent(
        ...
        if invalid_tools:
            print(f"⚠️ [AgentBuilderStudio] Herramientas no encontradas: {invalid_tools}")
        ...
        print(f"✅ [AgentBuilderStudio] Agente creado: {name} (ID: {agent_id})")
        return agent_id
    ...
    async def run_agent(self, agent_id: str, user_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"❌ [AgentBuilderStudio] Error ejecutando agente {agent_id}: {e}")
            import traceback
            traceback.print_exc()
    ...
    def deploy_agent(
        ...
        print(f"✅ [AgentBuilderStudio] Agente {agent_config.name} ({agent_id}) desplegado en: {[c.value for c in channels]}")
        if web_url:
    ...
    def delete_agent(self, agent_id: str) -> bool:
        ...
        if agent_id in self.agents:
            ...
            print(f"✅ [AgentBuilderStudio] Agente {agent_id} eliminado")
            return True
        return False
...
def get_agent_builder_studio(config: AppConfig) -> AgentBuilderStudio:
    """Obtiene instancia singleton de AgentBuilderStudio."""
    if not hasattr(get_agent_builder_studio, "_instance"):
        get_agent_builder_studio._instance = AgentBuilderStudio(config)
    return get_agent_builder_studio._instance


async def run_agent_builder_studio(
    message: str,
    ...
    """
    Ejecuta un agente del Agent Builder Studio.
    ...
    response = await studio.run_agent(agent_id, message)
