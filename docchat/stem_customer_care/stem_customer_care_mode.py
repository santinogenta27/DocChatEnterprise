"""STEM Customer Care - Modo principal.
...
from .agents.stem_customer_care_agent import StemCustomerCareAgent, StemCustomerCareConfig
...
class StemCustomerCareMode:
    """Modo principal STEM Customer Care."""
    ...
    def __init__(self, config: Optional[AppConfig] = None, llm: Optional[BaseLanguageModel] = None) -> None:
        ...
        if llm is not None:
            ...
            # SIEMPRE usar Groq para STEM Customer Care (como solicitado)
            groq_api_key = self.config.groq_api_key or os.getenv("GROQ_API_KEY")
            if not groq_api_key:
                raise ValueError("GROQ_API_KEY requerida para STEM Customer Care. Configura en .env: GROQ_API_KEY=tu-clave")
            
            try:
                from langchain_groq import ChatGroq
                # SIEMPRE usar Groq 3.3 70B para STEM Customer Care
                groq_llm = ChatGroq(
                ...
                self.llm = groq_llm
                print("✅ STEM Customer Care usando Groq (Llama 3.3 70B Versatile) - Velocidad <0.5 seg")
            except ImportError:
        ...
        if self.config.postgresql_enabled and self.config.postgresql_url:
            try:
                ...
                print("✅ STEM Customer Care usando PostgreSQL - Memoria de largo plazo activa")
            except Exception as e:
        ...
        self.agent = StemCustomerCareAgent(
            llm=self.llm,
    ...
    def process_message(self, payload: Dict[str, Any], channel: str = "web") -> Dict[str, Any]:
        ...
        # Detectar si es customer service o ventas
        # Por defecto, usar customer service profesional
        use_customer_service = payload.get("mode", "customer_service") == "customer_service"
        
        if use_customer_service:
            # Usar método de customer service profesional
            zendesk_ticket_id = payload.get("zendesk_ticket_id")
            result = self.agent.handle_customer_service_message(
                session=session,
                user_message=internal_msg.content,
                zendesk_ticket_id=zendesk_ticket_id,
            )
        else:
            # Usar método de ventas (modo original)
        ...
        external = adapter.to_external_response(response_text=response_text, extra={
            ...
            "case_id": result.get("case_id"),  # ID del caso si es customer service
            "status": result.get("status"),  # Estado del caso
        })
        return external
    ...
    def get_api_router(self) -> APIRouter:
        router = APIRouter(prefix="/stem-customer-care", tags=["STEM Customer Care"])

        @router.post("/chat")
        async def chat(payload: Dict[str, Any]) -> Dict[str, Any]:
            """Endpoint de chat para STEM Customer Care Widget"""
            try:
    ...
    def get_gradio_interface(self):  # type: ignore[override]
        ...
        return gr.Interface(fn=_chat_fn, inputs=["text", "text"], outputs="text", title="STEM Customer Care")
