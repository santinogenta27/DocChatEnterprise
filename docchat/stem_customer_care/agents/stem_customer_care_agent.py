...
from ..case_manager import CaseManager, Case, CaseStatus, CasePriority
from ..workflows.customer_service_workflow import CustomerServiceWorkflowManager
from ..integrations.zendesk_integration import ZendeskIntegration, ZendeskTicket


@dataclass
class StemCustomerCareConfig:
...
class StemCustomerCareAgent:
    """Agente orquestador de STEM Customer Care.
    ...
    def __init__(
        ...
        config: StemCustomerCareConfig | None = None,
        ...
        self.config = config or StemCustomerCareConfig()
        ...
        memory_storage_dir = Path(app_config.memory_dir if app_config else "memory") / "stem_customer_care" / "conversation_memory"
        ...
        cart_storage_dir = Path(app_config.memory_dir if app_config else "memory") / "stem_customer_care" / "abandoned_carts"
        ...
        analytics_storage_dir = Path(app_config.memory_dir if app_config else "memory") / "stem_customer_care" / "analytics"
        ...
        # 8. Sistema de Customer Service Profesional
        self.case_manager = CaseManager()
        self.workflow_manager = CustomerServiceWorkflowManager()
    ...
    def _initialize_rag(self):
        ...
        try:
            ...
            storage_dir = Path("docchat/stem_customer_care/rag_storage")
    ...
    def _build_rag_retriever_lazy(self):
        ...
        try:
            ...
            # Cargar metadata
            metadata_path = self._rag_metadata_path or Path("docchat/stem_customer_care/rag_storage/retriever_metadata.pkl")
            persist_dir = None
            ...
            if persist_dir and persist_dir.exists() and embeddings_generated:
                ...
                persist_dir = Path(config.persist_dir) / "stem_customer_care_rag"
    ...
    def _query_rag(self, query: str, top_k: int = 5) -> str:
        ...
        try:
            ...
            workflow = AgentWorkflow(
                ...
                provider="groq"  # SIEMPRE usar Groq para STEM Customer Care
            )
            ...
            # Retornar diccionario con contexto y fuentes
            return {
                "context": answer,
                "sources": sources
            }
            ...
            try:
                ...
                context_text = "\n\n---\n\n".join(context_parts)
                # Retornar diccionario con contexto y fuentes para customer service
                return {
    ...
    def handle_message(
        ...
        rag_context = ""
        if self.chatbot_config.rag_enabled:
            rag_result = self._query_rag(user_message, top_k=5)
    ...
    def handle_customer_service_message(
        self,
        session: CustomerSessionState,
        user_message: str,
        zendesk_ticket_id: Optional[int] = None,
    ) -> Dict[str, Any]:
        """Maneja mensajes de customer service con comportamiento profesional.
        ...
        1. Reconocer el problema (demostrar comprensión)
        2. Decir qué va a hacer
        3. Ejecutar o explicar el resultado
        4. Confirmar
        5. Cerrar
        
        LENGUAJE:
        - Humano, seguro, directo
        - Presente activo
        - Sin frases de bot
        - Sin emojis
        - Sin tecnicismos
        ...
        # === PASO 1: Obtener o crear caso ===
        case = None
        if zendesk_ticket_id and self.zendesk.enabled:
            # Obtener ticket de Zendesk
            zendesk_ticket = self.zendesk.get_ticket(zendesk_ticket_id)
            if zendesk_ticket:
                # Buscar caso existente o crear uno nuevo
                case = self.case_manager.get_case_by_email(zendesk_ticket.requester_email or session.profile.email if session.profile else "")
                if not case:
                    case = self.case_manager.create_case(
                        subject=zendesk_ticket.subject,
                        description=zendesk_ticket.description,
                        requester_email=zendesk_ticket.requester_email or session.profile.email if session.profile else session.session_id,
                        requester_name=zendesk_ticket.requester_name,
                        priority=CasePriority.HIGH if zendesk_ticket.priority == "urgent" else CasePriority.MEDIUM,
                    )
                    case.zendesk_ticket_id = zendesk_ticket_id
        else:
            # Buscar caso activo por email o crear uno nuevo
            requester_email = session.profile.email if session.profile and hasattr(session.profile, 'email') else None
            if requester_email:
                case = self.case_manager.get_case_by_email(requester_email)
            
            if not case:
                # Crear nuevo caso
                case = self.case_manager.create_case(
                    subject=user_message[:100] if len(user_message) > 100 else user_message,
                    description=user_message,
                    requester_email=requester_email or session.session_id,
                    requester_name=session.profile.display_name if session.profile else None,
                )
        
        case.add_message("user", user_message)
        
        # === PASO 2: Análisis de sentimiento ===
        sentiment_result = self.sentiment_analyzer.analyze(user_message, session)
        self.sentiment_analyzer.update_session_sentiment(session, sentiment_result)
        
        # === PASO 3: Consultar RAG (solo información conocida) ===
        rag_context = ""
        rag_sources = []
        if self.chatbot_config.rag_enabled:
            rag_result = self._query_rag(user_message, top_k=5)
            if rag_result:
                ...
                if isinstance(rag_result, dict):
                    ...
                    # Intentar extraer fuentes del texto si están presentes
                    import re
                    source_matches = re.findall(r'\[Fuente: ([^\]]+)\]', rag_context)
                    if source_matches:
                        rag_sources = source_matches
        
        # === PASO 4: Obtener workflow apropiado ===
        workflow = self.workflow_manager.get_workflow_for_case("generic")
        current_step = workflow.steps[0] if workflow.steps else None
        
        # === PASO 5: Decidir acción (resolver, escalar, o pedir información) ===
        should_escalate = self._should_escalate_case(case, session, user_message, rag_context)
        
        if should_escalate["should_escalate"]:
            # Escalar caso
            escalation_reason = should_escalate["reason"]
            case.escalate(escalation_reason)
            
            # Actualizar en Zendesk si está conectado
            if case.zendesk_ticket_id and self.zendesk.enabled:
                self.zendesk.update_ticket_status(case.zendesk_ticket_id, "open")
                self.zendesk.add_tags(case.zendesk_ticket_id, ["escalated", "ai_escalated"])
                self.zendesk.add_internal_note(
                    case.zendesk_ticket_id,
                    f"Escalado por AI Agent. Razón: {escalation_reason}\n\n"
                    f"Contexto: {should_escalate.get('context', '')}\n"
                    f"Recomendaciones: {should_escalate.get('recommendations', 'Revisar caso manualmente')}"
                )
            
            # Generar respuesta de escalación
            response_text = self._generate_escalation_response(case, escalation_reason)
            case.add_message("assistant", response_text)
            case.add_action("escalation", f"Caso escalado: {escalation_reason}")
        ...
        # === PASO 6: Intentar resolver el caso ===
        # Construir prompt profesional para customer service
        system_prompt = self._build_customer_service_prompt(case, session, rag_context, rag_sources, workflow)
        
        # Generar respuesta siguiendo la estructura obligatoria
        response_prompt = (
            f"**Mensaje del cliente:** {user_message}\n\n"
            "**INSTRUCCIONES OBLIGATORIAS DE RESPUESTA:**\n\n"
            "**1. PRIMER MENSAJE (si es el primero del caso):**\n"
            "- Resumir el problema del cliente\n"
            "- Demostrar que leíste y comprendiste el caso\n"
            "- NO pedir información que ya existe en el historial\n\n"
            "**2. ESTRUCTURA OBLIGATORIA (SIEMPRE):**\n"
            "- **Reconocer:** Demuestra comprensión del problema\n"
            "- **Actuar/Explicar:** Di qué vas a hacer y hazlo, o explica el resultado\n"
            "- **Confirmar:** Confirma que la acción se ejecutó o el estado actual\n"
            "- **Cerrar:** Cierra de forma clara y definitiva (evita preguntas abiertas)\n\n"
            "**3. LENGUAJE OBLIGATORIO:**\n"
            "- Humano, seguro, directo\n"
            "- Presente activo ('Voy a revisar' no 'Revisaré')\n"
            "- Sin frases de bot ('Lo siento por las molestias', 'Estoy aquí para ayudarte')\n"
            "- Sin emojis\n"
            "- Sin tecnicismos innecesarios\n"
            "- Sin disculpas innecesarias\n\n"
            "**4. USO DE CONOCIMIENTO:**\n"
            f"{'Usa SOLO la información de los documentos arriba. NO inventes.' if rag_context else 'No hay información en la base de conocimiento. Si no tienes información suficiente, dilo claramente.'}\n\n"
            "**5. GUARDRAILS:**\n"
            "- Nunca prometas lo que no controlas\n"
            "- Nunca culpes al cliente\n"
            "- Nunca muestres dudas\n"
            "- Nunca inventes información\n"
            "- Nunca repitas soluciones fallidas\n"
            "- Nunca delegues trabajo técnico al cliente si puedes hacerlo tú\n\n"
            "**6. CIERRE:**\n"
            "- Cierra de forma clara y definitiva\n"
            "- Evita preguntas abiertas al final\n"
            "- Si el problema está resuelto, cierra el caso\n"
            "- Si necesita seguimiento, di cuándo y cómo\n\n"
            "Genera la respuesta ahora siguiendo TODAS estas instrucciones:"
        )
        
        # Invocar LLM
        llm_response = self._invoke_llm_with_fallback([
            SystemMessage(content=system_prompt),
        ...
        # Limpiar respuesta (remover frases de bot comunes)
        response_text = self._clean_bot_phrases(response_text)
        
        # === PASO 7: Ejecutar acciones si es necesario ===
        actions_executed = []
        
        # Si la respuesta indica que se va a hacer algo, intentar hacerlo
        if "voy a" in response_text.lower() or "revisando" in response_text.lower():
            # Intentar acciones automáticas según el caso
            if "ticket" in user_message.lower() or "pedido" in user_message.lower():
                # Buscar información de pedidos
                pass  # TODO: Implementar búsqueda de pedidos
        
        # === PASO 8: Actualizar caso ===
        case.add_message("assistant", response_text)
        case.add_action("response", "Respuesta generada al cliente")
        
        # Verificar si el caso está resuelto
        if self._is_case_resolved(case, response_text):
            case.update_status(CaseStatus.RESOLVED, "Problema resuelto según respuesta del agente")
            
            # Actualizar en Zendesk
            if case.zendesk_ticket_id and self.zendesk.enabled:
                self.zendesk.update_ticket_status(case.zendesk_ticket_id, "solved")
                self.zendesk.add_tags(case.zendesk_ticket_id, ["resolved", "ai_resolved"])
        
        # === PASO 9: Guardar en sesión ===
        ...
        return {
            ...
            "sentiment": session.sentiment.value,
            "frustration_score": session.frustration_score,
            "needs_handoff": case.status == CaseStatus.ESCALATED,
    ...
    def _build_customer_service_prompt(
        ...
        """Construye el prompt del sistema para customer service profesional."""
        prompt = (
            f"Eres un operador senior de customer service de {self.config.brand_name}.\n\n"
            "**TU ROL:**\n"
            "- NO eres un chatbot. Eres un operador profesional.\n"
            "- Resuelves problemas reales de clientes.\n"
            "- Tomas control del caso.\n"
            "- Actúas, resuelves o escalas con criterio.\n"
            "- Cierras solo cuando corresponde.\n\n"
        )
        
        # Información del caso
        if case:
            prompt += f"**CASO ACTUAL:**\n"
            prompt += f"- ID: {case.case_id}\n"
            prompt += f"- Asunto: {case.subject}\n"
            prompt += f"- Estado: {case.status.value}\n"
            prompt += f"- Prioridad: {case.priority.value}\n"
            if case.messages:
                prompt += f"- Mensajes previos: {len(case.messages)}\n"
            if case.actions_taken:
                prompt += f"- Acciones tomadas: {len(case.actions_taken)}\n"
            prompt += "\n"
        
        # Historial del caso
        if case and case.messages:
            prompt += "**HISTORIAL DEL CASO:**\n"
            for msg in case.messages[-5:]:  # Últimos 5 mensajes
        ...
        # Información RAG
        if rag_context:
            prompt += f"**INFORMACIÓN DE BASE DE CONOCIMIENTO:**\n{rag_context}\n\n"
            if rag_sources:
                prompt += f"**Fuentes:** {', '.join(rag_sources)}\n\n"
            prompt += "**IMPORTANTE:** Usa SOLO esta información. NO inventes. Si no hay información suficiente, dilo claramente.\n\n"
        else:
            prompt += "**BASE DE CONOCIMIENTO:** No hay información disponible.\n"
            prompt += "Si no tienes información suficiente para resolver, dilo claramente y escala si es necesario.\n\n"
    ...
    def _should_escalate_case(
        ...
        """Determina si un caso debe ser escalado.
        ...
        if not rag_context and ("no sé" in user_message.lower() or "no tengo" in user_message.lower()):
            reasons.append("Falta información crítica en base de conocimiento")
            recommendations.append("Revisar y actualizar base de conocimiento")
        
        # 2. Sentimiento negativo alto
        if session.frustration_score >= 0.8 or session.sentiment.value == "negative":
            reasons.append(f"Sentimiento negativo alto (frustración: {session.frustration_score:.2f})")
            recommendations.append("Revisar caso con atención especial")
        
        # 3. Múltiples intentos fallidos
        if case and len(case.actions_taken) >= 3:
            failed_attempts = [a for a in case.actions_taken if a.get("result") == "failed"]
            if len(failed_attempts) >= 2:
                reasons.append("Múltiples intentos de resolución fallidos")
                recommendations.append("Revisar caso manualmente")
        
        # 4. Cliente VIP (detectar por historial)
        if hasattr(session, 'customer_id') and session.customer_id:
        ...
        return {
            ...
            "recommendations": "; ".join(recommendations) if recommendations else "Revisar caso manualmente",
        }
    
    def _generate_escalation_response(self, case: Case, reason: str) -> str:
        """Genera respuesta de escalación profesional."""
        return (
            f"Entiendo tu situación. Voy a escalar tu caso a nuestro equipo especializado.\n\n"
            f"Un agente humano revisará tu caso y se pondrá en contacto contigo pronto.\n\n"
            f"Tu caso ha sido marcado como prioritario. Recibirás una respuesta en breve."
        )
    
    def _clean_bot_phrases(self, text: str) -> str:
        """Limpia frases típicas de bot del texto."""
        bot_phrases = [
    ...
    def _is_case_resolved(self, case: Case, response_text: str) -> bool:
        """Determina si un caso está resuelto basándose en la respuesta."""
        resolved_indicators = [
            "resuelto",
            "solucionado",
            "completado",
        ...
        # Si menciona que está resuelto o cerrado
        if any(indicator in text_lower for indicator in resolved_indicators + closing_indicators):
            # Verificar que no sea una pregunta
            if "?" not in response_text[-50:]:  # Últimos 50 caracteres
                return True
