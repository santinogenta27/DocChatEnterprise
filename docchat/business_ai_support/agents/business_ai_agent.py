...
class BusinessAIAgent:
    ...
    def __init__(
        ...
        self.crm_tool = crm_tool  # Deep CRM integration
        ...
        memory_storage_dir = Path(app_config.memory_dir if app_config else "memory") / "business_ai_support" / "conversation_memory"
        ...
        cart_storage_dir = Path(app_config.memory_dir if app_config else "memory") / "business_ai_support" / "abandoned_carts"
        ...
        analytics_storage_dir = Path(app_config.memory_dir if app_config else "memory") / "business_ai_support" / "analytics"
    ...
    def _initialize_rag(self):
        ...
        try:
            ...
            storage_dir = Path("docchat/business_ai_support/rag_storage")
    ...
    def _build_rag_retriever_lazy(self):
        ...
        try:
            ...
            # Cargar metadata
            metadata_path = self._rag_metadata_path or Path("docchat/business_ai_support/rag_storage/retriever_metadata.pkl")
            persist_dir = None
    ...
    def _trigger_human_handoff(self, session: CustomerSessionState, reason: str, user_message: str):
        ...
        if self.crm_tool and self.crm_tool.has_crm:
            try:
                customer_email = None
                if session.profile and hasattr(session.profile, 'email'):
                    customer_email = session.profile.email
                elif session.profile and isinstance(session.profile, dict):
                    customer_email = session.profile.get('email')
                
                if customer_email:
                    crm_case = self.crm_tool.create_crm_case(
                        case_data={
                            "subject": ticket.get("subject", f"Handoff Automático: {reason}"),
                            "description": ticket.get("description", user_message[:500]),
                            "status": "open",
                            "priority": "high"
                        },
                        contact_email=customer_email
                    )
                    if crm_case and crm_case.get("success"):
                        print(f"✅ Ticket sincronizado con CRM: {crm_case.get('case_id')}")
                        # Store CRM case ID in ticket metadata
                        if isinstance(ticket, dict):
                            ticket["crm_case_id"] = crm_case.get("case_id")
                            ticket["crm_provider"] = crm_case.get("provider")
            except Exception as e:
    ...
    def handle_message(
        ...
        # === CRM INTEGRATION: Get Customer Context from CRM ===
        crm_context = ""
        crm_contact_id = None
        if self.crm_tool and self.crm_tool.has_crm:
            try:
                # Try to get customer info from CRM by email
                customer_email = None
                if session.profile and hasattr(session.profile, 'email'):
                    customer_email = session.profile.email
                elif session.profile and isinstance(session.profile, dict):
                ...
                if customer_email:
                    crm_info = self.crm_tool.get_customer_info(customer_email)
                    if crm_info and crm_info.get("found"):
                        crm_contact_id = crm_info.get("contact_id")
                        crm_data = crm_info.get("data", {})
                        
                        # Build CRM context for the agent
                        crm_context = f"\n**📋 INFORMACIÓN DEL CLIENTE DESDE CRM:**\n"
                        if crm_data.get("name") or crm_data.get("FirstName") or crm_data.get("firstname"):
                            name = crm_data.get("name") or f"{crm_data.get('FirstName', '')} {crm_data.get('LastName', '')}".strip() or crm_data.get("firstname", "")
                            crm_context += f"- Nombre: {name}\n"
                        if crm_data.get("Email") or crm_data.get("email"):
                            crm_context += f"- Email: {crm_data.get('Email') or crm_data.get('email')}\n"
                        if crm_data.get("Phone") or crm_data.get("phone"):
                            crm_context += f"- Teléfono: {crm_data.get('Phone') or crm_data.get('phone')}\n"
                        if crm_data.get("Company") or crm_data.get("company_name"):
                            crm_context += f"- Empresa: {crm_data.get('Company') or crm_data.get('company_name')}\n"
                        
                        # Get customer history
                        history = self.crm_tool.get_customer_history(crm_contact_id)
                        if history and history.get("history_count", 0) > 0:
                            crm_context += f"- Historial: {history['history_count']} interacciones previas\n"
                            # Include recent cases/tickets
                            recent_records = history.get("records", [])[:3]
                            if recent_records:
                                crm_context += "  - Casos/Tickets recientes:\n"
                                for record in recent_records:
                                    if record.get("type") in ["case", "ticket"]:
                                        subject = record.get("data", {}).get("Subject") or record.get("data", {}).get("subject", "N/A")
                                        status = record.get("data", {}).get("Status") or record.get("data", {}).get("status", "N/A")
                                        crm_context += f"    • {subject} (Estado: {status})\n"
                        
                        crm_context += "\n"
                        print(f"✅ Información CRM obtenida para contacto: {crm_contact_id}")
                        
            except Exception as e:
                print(f"⚠️ Error obteniendo información CRM: {e}")
                crm_context = ""
        ...
        # === CRM INTEGRATION: Create or Update Contact in CRM ===
        if self.crm_tool and self.crm_tool.has_crm and session.profile:
            try:
                # Try to get or create contact in CRM
                customer_email = None
                customer_name = None
                customer_phone = None
                
                if hasattr(session.profile, 'email'):
                    customer_email = session.profile.email
                    customer_name = getattr(session.profile, 'name', None)
                    customer_phone = getattr(session.profile, 'phone', None)
                elif isinstance(session.profile, dict):
                    customer_email = session.profile.get('email')
                    customer_name = session.profile.get('name')
                    customer_phone = session.profile.get('phone')
                
                if customer_email:
                    # Create or update contact with latest info
                    contact_data = {"email": customer_email}
                    if customer_name:
                        contact_data["name"] = customer_name
                    if customer_phone:
                        contact_data["phone"] = customer_phone
                    
                    # Update with session sentiment and lead score if available
                    if hasattr(session, 'sentiment'):
                        contact_data["sentiment"] = session.sentiment.value if hasattr(session.sentiment, 'value') else str(session.sentiment)
                    if hasattr(session, 'lead_score'):
                        contact_data["lead_score"] = session.lead_score
                    
                    crm_contact = self.crm_tool.create_or_update_contact(contact_data)
                    if crm_contact and crm_contact.get("action"):
                        print(f"✅ Contacto {'creado' if crm_contact['action'] == 'created' else 'actualizado'} en CRM: {crm_contact.get('contact_id')}")
            except Exception as e:
                print(f"⚠️ Error sincronizando contacto con CRM: {e}")
        ...
        system_prompt = (
            f"Eres el agente oficial de soporte de {self.config.brand_name}. "
            "Tu especialidad es **SOPORTE AL CLIENTE 24/7** con capacidades avanzadas:\n\n"
            "**🎫 GESTIÓN DE TICKETS:**\n"
            "- Puedes crear tickets automáticamente cuando detectas problemas\n"
            ...
            "- Puedes guiar a clientes paso a paso para resolver problemas\n"
            "- Haces preguntas secuenciales para diagnosticar\n"
            "- Usas RAG para información técnica en cada paso\n\n"
            "**📅 AGENDAMIENTO INTERNO:**\n"
            "- Puedes programar citas y reuniones\n"
            "- Consultar disponibilidad en tiempo real\n"
            "- Confirmar y cancelar citas\n\n"
            "**Herramientas disponibles:** tickets, troubleshooting, scheduling, RAG, catálogo, pedidos.\n\n"
        )
        ...
        analysis_prompt = (
            ...
            "{\n"
            "  'intent': 'sales' | 'support' | 'order_status' | 'refund' | 'troubleshooting' | 'scheduling' | 'small_talk',\n"
            ...
            "  'troubleshooting_guide_id': 'opcional - ID de guía a iniciar',\n"
            "  'needs_scheduling': true/false,\n"
            "  'appointment_type': 'opcional - tipo de cita',\n"
        ...
        # Devoluciones / soporte / tickets
        if intent in ("support", "refund") or intent_data.get("needs_ticket"):
            user_id = session.customer_id if hasattr(session, 'customer_id') else session.session_id
            ticket = self.support_tool.create_ticket(
                session_id=session_id,
                subject=f"Soporte: {intent or 'Consulta'}",
                description=user_message[:1000],
                priority="normal" if intent == "support" else "high",
                user_id=user_id
            )
            if not hasattr(session, 'open_tickets'):
                session.open_tickets = []
            session.open_tickets.append(ticket)
            ...
            if self.crm_tool and self.crm_tool.has_crm:
                try:
                    ...
                    if customer_email:
                        crm_case = self.crm_tool.create_crm_case(
                            case_data={
                                "subject": ticket.get("subject", f"Soporte: {intent or 'Consulta'}"),
                                "description": ticket.get("description", user_message[:1000]),
                                "status": "open",
        ...
        # Troubleshooting (guías paso a paso)
        if (intent == "troubleshooting" or intent_data.get("needs_troubleshooting")) and self.troubleshooting_engine:
            guide_id = intent_data.get("troubleshooting_guide_id")
            if guide_id:
                # Iniciar guía específica
                guide_result = self.troubleshooting_engine.start_guide(session_id, guide_id)
                if guide_result.get("success"):
                    tool_results["troubleshooting"] = guide_result
            else:
                # Buscar guía apropiada basada en el mensaje
                # Por ahora, usar guía de ejemplo si existe
                available_guides = list(self.troubleshooting_engine.guides.keys())
                if available_guides:
                    guide_result = self.troubleshooting_engine.start_guide(session_id, available_guides[0])
                    if guide_result.get("success"):
                        tool_results["troubleshooting"] = guide_result
        
        # Scheduling (agendamiento)
        if (intent == "scheduling" or intent_data.get("needs_scheduling")) and self.internal_scheduler:
            appointment_type = intent_data.get("appointment_type", "consultation")
            # Obtener slots disponibles
            from datetime import datetime, timedelta
            available_slots = self.internal_scheduler.get_available_slots(
                start_date=datetime.now(),
                end_date=datetime.now() + timedelta(days=7),
                appointment_type=appointment_type
            )
            tool_results["available_slots"] = [
                {
                ...
                for slot in available_slots[:10]  # Limitar a 10 slots
            ]
