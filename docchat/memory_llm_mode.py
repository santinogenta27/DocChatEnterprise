"""
Memory LLM Mode - Sistema Multi-Agente RAG con Memoria Persistente
Integra el sistema completo de DocChat Multi-Agent RAG con memoria de largo plazo:
...
class MemoryLLMMode:
    """
    Memory LLM Mode - Sistema Multi-Agente RAG con Memoria Persistente de Largo Plazo.
    ...
    def __init__(
        ...
        if not config.openai_api_key:
            raise ValueError("OPENAI_API_KEY requerida para Memory LLM Mode")
        ...
        try:
            ...
            print("✅ [Memory LLM Mode] CrewAI integrado para evaluación de documentos")
        except Exception as e:
            print(f"⚠️ [Memory LLM Mode] CrewAI no disponible: {e}")
            self.crewai = None
    ...
    def process_documents(
        ...
        try:
            print(f"📄 [Memory LLM Mode] Procesando {len(new_files)} nuevos documentos...")
            ...
            if session["docs"]:
                ...
                print(f"✅ [Memory LLM Mode] Retriever actualizado: {len(session['docs'])} chunks")
            ...
            print(f"❌ [Memory LLM Mode] Error procesando documentos: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    async def process_query_async(
        ...
        try:
            ...
            print(f"⚠️ [Memory LLM Mode] Error agregando pasos de razonamiento: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Memory LLM Mode] Error en Reinforcement Planning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Memory LLM Mode] Error en Path-dependent Reasoning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Memory LLM Mode] Error consultando MCP: {e}")
        ...
        try:
            ...
            for conn_id, connection in self.mcp_manager.connections.items():
                ...
                try:
                    ...
                    print(f"⚠️ [Memory LLM Mode] Error consultando MCP {connection.name}: {e}")
                    continue
            ...
            print(f"⚠️ [Memory LLM Mode] Error en consulta MCP: {e}")
            return None
...
RESPUESTA ESPECÍFICA A LA PREGUNTA DEL USUARIO (300-500 palabras):"""
        ...
        print(f"🔄 [Memory LLM Mode] Procesando {len(docs_by_source)} documentos en paralelo...")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"✅ [Memory LLM Mode] Análisis completado para: {Path(doc_name).name}")
                except Exception as e:
                    print(f"❌ [Memory LLM Mode] Error procesando {source_name}: {e}")
...
RESPUESTA FINAL QUE RESPONDE DIRECTAMENTE LA PREGUNTA DEL USUARIO (800-1200 palabras):"""
    ...
    def _create_evaluation_agent(self, agent_type: str) -> Optional[Any]:
        ...
        try:
            ...
            print(f"⚠️ [Memory LLM Mode] Error creando agente {agent_type}: {e}")
            return None
    ...
    async def _evaluate_section_with_agent(
        ...
        try:
            ...
            print(f"⚠️ [Memory LLM Mode] Error en evaluación con CrewAI: {e}")
    ...
    async def _evaluate_section_with_llm(
        ...
        try:
            ...
            print(f"⚠️ [Memory LLM Mode] Error en evaluación con LLM: {e}")
            return {
                "score": 0,
...
{{
    ...
    async def evaluate_document(
        ...
        print(f"📊 [Memory LLM Mode] Iniciando evaluación de {len(docs_to_evaluate)} documento(s)...")
        ...
        for doc in docs_to_evaluate:
            ...
            print(f"📄 [Memory LLM Mode] Documento segmentado en {len(sections)} secciones")
            ...
            for section in sections:
                print(f"🔍 [Memory LLM Mode] Evaluando sección: {section['name']}")
                section_evaluation = {
                ...
                for agent_type in agent_types:
                    try:
                        ...
                        print(f"⚠️ [Memory LLM Mode] Error evaluando sección {section['name']} con {agent_type}: {e}")
            ...
            print(f"✅ [Memory LLM Mode] Evaluación completada para documento: {doc_metadata.get('source', 'unknown')}")
...
# Instancia global
_memory_llm_mode_instance: Optional[MemoryLLMMode] = None


def get_memory_llm_mode(
    config: AppConfig,
    processor: DocumentProcessor,
    retriever_builder: RetrieverBuilder,
    context_manager: Optional[Any] = None
) -> MemoryLLMMode:
    """Obtiene o crea la instancia global de Memory LLM Mode."""
    global _memory_llm_mode_instance
    
    if _memory_llm_mode_instance is None:
        _memory_llm_mode_instance = MemoryLLMMode(
            config=config,
            processor=processor,
    ...
    return _memory_llm_mode_instance


def run_memory_llm_mode(
    message: str,
    history: List[Tuple[str, str]],
    files: List[Any],
    session_id: str,
    ...
    """
    Función principal para ejecutar Memory LLM Mode.
    ...
    if files:
        result = memory_llm_mode.process_documents(session_id, files)
    ...
    try:
        ...
        new_history, error, metadata = loop.run_until_complete(
            memory_llm_mode.process_query_async(
                session_id=session_id,
                message=message,
                history=history,
                speed_mode=speed_mode,
