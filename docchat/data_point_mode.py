"""
Data Point Mode - Sistema Event-Driven Multi-Agente RAG
...
- 📡 Data Point: Sistema de mensajería interna para comunicación entre componentes
...
from .neusym_rag import NeuSymRAG, ActionType, Action, Observation
from .multiview_chunking import MultiviewChunker, MultiviewChunks, ChunkingView
from .multi_strategy_parsing import MultiStrategyParser, ParsingStrategy, DocumentFormat, Node


class SimpleEventBus:
    """Data Point optimizado para real-time streaming y procesamiento asíncrono."""
    ...
    def publish(self, event_type: str, data: Dict[str, Any], async_notify: bool = True):
        ...
        for callback in self.subscribers[event_type]:
            try:
                ...
                print(f"⚠️ [Data Point] Error en callback para {event_type}: {e}")
    ...
    async def _notify_async_subscribers(self, event_type: str, data: Dict[str, Any]):
        ...
        for callback in self.async_subscribers[event_type]:
            try:
                ...
                print(f"⚠️ [Data Point] Error en async callback para {event_type}: {e}")
...
class DataPointMode:
    """
    Data Point Mode - Sistema Event-Driven Multi-Agente RAG de Máxima Calidad para Empresas.
    ...
    ARQUITECTURA EVENT-DRIVEN:
    - Data Point interno para comunicación entre componentes
    ...
    def __init__(
        ...
        if not config.openai_api_key:
            raise ValueError("OPENAI_API_KEY requerida para Data Point Mode")
        ...
        # NeuSym-RAG: Hybrid Neural-Symbolic Retrieval
        self.neusym_rag: Optional[NeuSymRAG] = None
        
        # Multiview Chunking
        self.multiview_chunker = MultiviewChunker(
            llm=self.llm,
            chunk_size=512,
            chunk_overlap=50
        )
    ...
    def _publish_event(self, event_type: str, data: Dict[str, Any], async_notify: bool = True):
        """Publica en Data Point y opcionalmente en Kafka."""
    ...
    def _on_document_uploaded(self, data: Dict[str, Any]):
        ...
        print(f"📄 [Data Point] Documento subido detectado: {file_name}")
        ...
        try:
            ...
            print(f"⚠️ [Data Point] Error en auto-procesamiento de documento: {e}")
    ...
    def _on_query_completed(self, data: Dict[str, Any]):
        ...
        print(f"✅ [Data Point] Query completado: {query}...")
    ...
    def _on_error_occurred(self, data: Dict[str, Any]):
        ...
        print(f"❌ [Data Point] Error en {context}: {error}")
    ...
    def _on_new_compliance_doc(self, data: Dict[str, Any]):
        ...
        print(f"🔍 [Data Point] Nuevo documento de compliance detectado: {file_path}")
    ...
    def _on_document_updated(self, data: Dict[str, Any]):
        ...
        print(f"🔄 [Data Point] Documento actualizado: {doc_id} desde {source} ({change_type})")
    ...
    def _on_compliance_complete(self, data: Dict[str, Any]):
        ...
        print(f"✅ [Data Point] Compliance check completado: {entity_name} (Risk: {risk_score})")
    ...
    def _on_high_risk_detected(self, data: Dict[str, Any]):
        ...
        print(f"⚠️ [Data Point] ALTO RIESGO detectado: {entity_name} (Risk: {risk_score})")
    ...
    def _on_query_executed(self, data: Dict[str, Any]):
        ...
        print(f"💬 [Data Point] Query ejecutado: {query[:50]}...")
    ...
    async def _process_documents_async(
        ...
        Integra Multi-Strategy Parsing y Multiview Chunking de los papers.
        """
        try:
            print(f"📄 [Data Point Mode] Procesando {len(new_files)} nuevos documentos con Multi-Strategy Parsing...")
            
            # Publicar evento de inicio
            self._publish_event('document_processing_started', {
                'session_id': session_id,
                'file_count': len(new_files),
                'strategy': 'multi_strategy_parsing'
            })
            ...
            for file_obj in new_files:
                try:
                    ...
                    # Determinar formato del documento
                    ...
                    if file_name.endswith('.docx'):
                        ...
                        doc_format = DocumentFormat.PNG
                    
                    # 1. Multi-Strategy Parsing (FAST, LLM, OCR)
                    parsing_results = self.multi_strategy_parser.parse_document(file_path, doc_format)
                    
                    # 2. Ensamblar resultados de múltiples estrategias
                    assembled_markdown = self.multi_strategy_parser.assemble_results(parsing_results)
                    
                    # 3. Crear estructura de nodos jerárquica
                    nodes = self.multi_strategy_parser.create_node_structure(assembled_markdown, parsing_results)
                    
                    # 4. Convertir a Documentos de LangChain para compatibilidad
                    file_docs = []
                    for node in nodes:
                        doc = Document(
                    ...
                    # 5. Multiview Chunking (pages, sections, tables, images, formulas, chunks)
                    paper_id = f"paper_{session_id}_{processed_count}"
                    multiview_chunks = self.multiview_chunker.chunk_document(
                        paper_id=paper_id,
                        documents=file_docs,
                        metadata={"source": file_path, "format": doc_format.value}
                    )
                    
                    # 6. Generar resúmenes para diferentes vistas
                    multiview_chunks = self.multiview_chunker.generate_summaries(
                        multiview_chunks,
                        llm=self.llm
                    )
                    
                    # 7. Guardar multiview chunks en sesión
                    if "multiview_chunks" not in session:
                        session["multiview_chunks"] = {}
                    session["multiview_chunks"][paper_id] = multiview_chunks
                    
                    # 8. Inicializar NeuSym-RAG si no existe
                    if not self.neusym_rag:
                        self.neusym_rag = NeuSymRAG(
                            config=self.config,
                            llm=self.llm,
                            vectorstore=session.get("retriever"),
                            db_path=str(Path(self.config.audit_log_dir) / "neusym_rag.db")
                        )
                    
                    # 9. Parsear PDF a base de datos (NeuSym-RAG)
                    multiview_dict = {
                        "pages": [
                            {
                                ...
                                "content": p.content,
                                "summary": p.summary
                            } for p in multiview_chunks.pages
                        ],
                        "sections": [
                            {
                            ...
                            } for s in multiview_chunks.sections
                        ],
                        "tables": [
                            {
                            ...
                            } for t in multiview_chunks.tables
                        ],
                        "images": [
                            {
                            ...
                            } for img in multiview_chunks.images
                        ],
                        "equations": [
                            {
                            ...
                            } for eq in multiview_chunks.formulas
                        ],
                        "chunks": [
                            {
                            ...
                            } for c in multiview_chunks.chunks
                        ]
                    }
                    
                    self.neusym_rag.parse_pdf_to_database(
                        paper_id=paper_id,
                        pdf_path=file_path,
                        metadata={"title": file_name, "source": file_path},
                        multiview_chunks=multiview_dict
                    )
                    
                    # 10. Codificar columnas encodables al vectorstore
                    encodable_columns = [
                        ("pages", "page_summary"),
                    ...
                    self.neusym_rag.encode_to_vectorstore(paper_id, encodable_columns)
                    ...
                    print(f"⚠️ [Data Point Mode] Error procesando archivo: {e}")
            ...
            print(f"✅ [Data Point Mode] Procesamiento completado: {processed_count} documentos")
            
        except Exception as e:
            print(f"❌ [Data Point Mode] Error en procesamiento asíncrono: {e}")
    ...
    async def _generate_realtime_insights(self, session_id: str, doc: Document):
        ...
        try:
            ...
            print(f"⚠️ [Data Point Mode] Error generando insight: {e}")
    ...
    async def stream_realtime_insights(self, session_id: str) -> AsyncIterator[Dict[str, Any]]:
        ...
        Usage:
            async for insight in data_point_mode.stream_realtime_insights(session_id):
                print(insight)
        """
        last_index = 0
    ...
    async def _stream_query_insights(
        ...
        try:
            ...
            print(f"⚠️ [Data Point Mode] Error en streaming de insights: {e}")
    ...
    async def process_query_async(
        ...
        try:
            ...
            print(f"⚠️ [Data Point Mode] Error agregando pasos de razonamiento: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Data Point Mode] Error en Reinforcement Planning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Data Point Mode] Error en Path-dependent Reasoning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Data Point Mode] Error consultando MCP: {e}")
        ...
        try:
            # Intentar usar NeuSym-RAG (Hybrid Neural-Symbolic Retrieval) si está disponible
            use_neusym_rag = self.neusym_rag is not None and session.get("multiview_chunks")
            
            if use_neusym_rag:
                # Usar NeuSym-RAG con iterative retrieval (ReAct framework)
                print(f"🧠 [Data Point Mode] Usando NeuSym-RAG con iterative retrieval...")
                
                # Determinar formato de respuesta
                answer_format = "Provide a comprehensive answer based on the retrieved context."
                
                # Ejecutar iterative retrieval
                neusym_result = self.neusym_rag.iterative_retrieval(
                    question=message,
                    answer_format=answer_format,
                    max_interactions=20
                )
                
                answer = neusym_result.get("answer", "No se pudo generar respuesta.")
                interactions = neusym_result.get("interactions", [])
                turns = neusym_result.get("turns", 0)
                
                # Extraer fuentes de las interacciones
                sources = []
                ...
                verification_report = f"NeuSym-RAG: {turns} interacciones, {len(interactions)} acciones ejecutadas"
                relevance_label = "RELEVANT" if answer else "UNKNOWN"
                
                # Publicar evento de NeuSym-RAG
                self._publish_event('neusym_rag_completed', {
                    'session_id': session_id,
                    'query': message,
                    'turns': turns,
            ...
            formatted_answer += "- 📡 **Data Point:** Arquitectura event-driven para procesamiento automático\n"
        ...
        try:
            ...
            for conn_id, connection in self.mcp_manager.connections.items():
                ...
                try:
                    ...
                    print(f"⚠️ [Data Point Mode] Error consultando MCP {connection.name}: {e}")
                    continue
            ...
            print(f"⚠️ [Data Point Mode] Error en consulta MCP: {e}")
            return None
    ...
    def handle_webhook_event(self, event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"📡 [Data Point] Webhook recibido: {event_type}")
            ...
            print(f"❌ [Data Point] Error procesando webhook: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    def sync_document_source(self, source: str, interval: int = 60) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"🔄 [Data Point] Sincronizando desde {source} (intervalo: {interval}s)")
            ...
            print(f"❌ [Data Point] Error en sync: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    def subscribe_to_external_webhook(self, source: str, callback_url: str) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"🔔 [Data Point] Suscribiendo a webhooks de {source} -> {callback_url}")
            ...
            print(f"❌ [Data Point] Error suscribiendo webhook: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    async def _process_query_parallel(
        ...
        Mantiene toda la funcionalidad específica de Data Point Mode (NeuSym-RAG, multiview structuring).
        """
        ...
        def analyze_single_document(source_name: str, file_docs: List[Document]) -> Tuple[str, str]:
            """Analiza un solo documento con el prompt del usuario (Data Point Mode)."""
            try:
                doc_content = "\n\n".join([doc.page_content for doc in file_docs])
                if len(doc_content) > 4000:
                    doc_content = doc_content[:4000] + "..."
                
                # Prompt adaptado para Data Point Mode (NeuSym-RAG y multiview structuring)
                prompt = f"""Eres un analista estratégico de nivel C-Suite especializado en análisis con NeuSym-RAG (Hybrid Neural-Symbolic Retrieval) y multiview structuring. Tu tarea es analizar ESTE documento específico para responder DIRECTAMENTE la pregunta del usuario, usando el formato específico de Data Point Mode.
...
INSTRUCCIONES CRÍTICAS PARA DATA POINT MODE:
...
2. NEUSYM-RAG & MULTIVIEW STRUCTURING (formato específico de Data Point Mode):
   - 🧠 **NeuSym-RAG:** Combina retrieval neural (vectorstore) y simbólico (SQL DB) para máxima precisión
   - 📚 **Multiview Structuring:** Analiza el documento en múltiples vistas (secciones, tablas, figuras, fórmulas, referencias)
   - 🔍 **Extracción Multiview:** Identifica información desde diferentes perspectivas estructurales
   - 📊 **Estructura del Documento:** Analiza páginas, secciones, tablas, imágenes, fórmulas, referencias

3. ANÁLISIS ESPECÍFICO PARA ESTE DOCUMENTO:
   ...
   - Menciona qué vistas del documento (secciones, tablas, etc.) fueron más relevantes

4. RESPUESTA ESTRUCTURADA (300-500 palabras):
   - **Respuesta Directa** (1-2 párrafos): Responde la pregunta del usuario usando información extraída con NeuSym-RAG
   - **Análisis Multiview** (1-2 párrafos): Detalles de cómo diferentes vistas del documento (secciones, tablas, etc.) aportan información
   - **Insights NeuSym-RAG** (1 párrafo): Insights obtenidos mediante la combinación de retrieval neural y simbólico

5. FORMATO ESPECÍFICO DE DATA POINT MODE:
   - Menciona el uso de NeuSym-RAG (Hybrid Neural-Symbolic Retrieval)
   - Referencia las vistas multiview utilizadas (secciones, tablas, figuras, etc.)
   - Proporciona insights que emergen de la combinación de diferentes vistas estructurales

RESPUESTA ESPECÍFICA A LA PREGUNTA DEL USUARIO CON NEUSYM-RAG Y MULTIVIEW STRUCTURING (300-500 palabras):"""
        ...
        print(f"🔄 [Data Point Mode] Procesando {len(docs_by_source)} documentos en paralelo...")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"✅ [Data Point Mode] Análisis completado para: {Path(doc_name).name}")
                    ...
                    print(f"❌ [Data Point Mode] Error procesando {source_name}: {e}")
        ...
        synthesis_prompt = f"""Eres un consultor estratégico senior de nivel C-Suite especializado en análisis con NeuSym-RAG (Hybrid Neural-Symbolic Retrieval) y multiview structuring. Has analizado {len(individual_analyses)} documentos individualmente, cada uno respondiendo la pregunta del usuario usando el formato específico de Data Point Mode.

TU TAREA PRINCIPAL: Combinar todos los análisis individuales para responder DIRECTAMENTE la pregunta del usuario de manera completa y estratégica, manteniendo el formato específico de Data Point Mode.

PREGUNTA ESPECÍFICA DEL USUARIO (RESPONDE EXACTAMENTE ESTO):
{message}

ANÁLISIS INDIVIDUALES DE CADA DOCUMENTO (cada uno ya respondió la pregunta del usuario con NeuSym-RAG y multiview structuring):
{combined_context}

INSTRUCCIONES PARA RESPUESTA FINAL COMBINADA (800-1200 palabras) - FORMATO DATA POINT MODE:
...
2. SÍNTESIS NEUSYM-RAG & MULTIVIEW STRUCTURING (formato Data Point Mode):
   - 🧠 **NeuSym-RAG Consolidado:** Combina insights obtenidos mediante retrieval neural y simbólico de todos los documentos
   - 📚 **Multiview Structuring Consolidado:** Analiza cómo diferentes vistas (secciones, tablas, figuras, fórmulas) de todos los documentos aportan información
   - 🔍 **Extracción Multiview Consolidada:** Identifica información desde diferentes perspectivas estructurales de todos los documentos
   - 📊 **Estructura Consolidada:** Analiza páginas, secciones, tablas, imágenes, fórmulas, referencias de todos los documentos

3. INSIGHTS NEUSYM-RAG CONSOLIDADOS:
   - Proporciona insights que emergen de la combinación de retrieval neural y simbólico de todos los documentos
   - Identifica patrones que solo son visibles mediante la combinación de diferentes vistas estructurales
   - Menciona qué vistas del documento (secciones, tablas, etc.) fueron más relevantes en cada documento
...
5. FORMATO ESPECÍFICO DE DATA POINT MODE:
   - Menciona el uso de NeuSym-RAG (Hybrid Neural-Symbolic Retrieval) en la síntesis
   - Referencia las vistas multiview utilizadas (secciones, tablas, figuras, etc.) de todos los documentos
   - Proporciona insights que emergen de la combinación de diferentes vistas estructurales

RESPUESTA FINAL QUE RESPONDE DIRECTAMENTE LA PREGUNTA DEL USUARIO CON NEUSYM-RAG Y MULTIVIEW STRUCTURING CONSOLIDADOS (800-1200 palabras):"""
...
# Instancia global
_data_point_mode_instance: Optional[DataPointMode] = None


def get_data_point_mode(
    config: AppConfig,
    processor: DocumentProcessor,
    retriever_builder: RetrieverBuilder,
    context_manager: Optional[Any] = None
) -> DataPointMode:
    """Obtiene o crea la instancia global de Data Point Mode."""
    global _data_point_mode_instance
    
    if _data_point_mode_instance is None:
        _data_point_mode_instance = DataPointMode(
            config=config,
            processor=processor,
    ...
    return _data_point_mode_instance


def run_data_point_mode(
    message: str,
    history: List[Tuple[str, str]],
    files: List[Any],
    session_id: str,
    ...
    """
    Función principal para ejecutar Data Point Mode.
    ...
    # Obtener instancia
    data_point_mode = get_data_point_mode(
        config=config,
        processor=processor,
        retriever_builder=retriever_builder,
        context_manager=context_manager
    ...
    if files:
        result = data_point_mode.process_documents(session_id, files)
    ...
    try:
        ...
        new_history, error, metadata = loop.run_until_complete(
            data_point_mode.process_query_async(
                session_id=session_id,
                message=message,
                history=history,
                speed_mode=speed_mode,
