...
from .agents.sales_ai_agent import SalesAIAgent, SalesAIConfig
...
class SalesAIAgentMode:
    """Modo principal Sales AI Agent."""
    ...
    def __init__(self, config: Optional[AppConfig] = None, llm: Optional[BaseLanguageModel] = None) -> None:
        ...
        if llm is not None:
            ...
            if not groq_api_key:
                raise ValueError("GROQ_API_KEY requerida para Sales AI Agent. Configura en .env: GROQ_API_KEY=tu-clave")
            
            try:
                from langchain_groq import ChatGroq
                # SIEMPRE usar Groq 3.3 70B para Sales AI Agent
                groq_llm = ChatGroq(
                ...
                self.llm = groq_llm
                print("✅ Sales AI Agent usando Groq (Llama 3.3 70B Versatile) - Velocidad <0.5 seg")
            except ImportError:
        ...
        if self.config.postgresql_enabled and self.config.postgresql_url:
            try:
                ...
                print("✅ Sales AI Agent usando PostgreSQL - Memoria de largo plazo activa")
            except Exception as e:
    ...
    def get_api_router(self) -> APIRouter:
        router = APIRouter(prefix="/sales-ai-agent", tags=["Sales AI Agent"])

        @router.post("/chat")
        async def chat(payload: Dict[str, Any]) -> Dict[str, Any]:
            """Endpoint de chat para Sales AI Agent Widget"""
            try:
