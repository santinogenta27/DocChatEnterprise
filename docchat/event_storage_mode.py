"""
Event Storage Mode - Sistema Event-Driven Multi-Agente RAG
...
- 📡 Event Storage: Sistema de mensajería interna para comunicación entre componentes
...
from .enterprise_connectors import EnterpriseConnectorManager, ConnectorConfig


class SimpleEventBus:
    """Event Storage optimizado para real-time streaming y procesamiento asíncrono."""
    ...
    def publish(self, event_type: str, data: Dict[str, Any], async_notify: bool = True):
        ...
        for callback in self.subscribers[event_type]:
            try:
                ...
                print(f"⚠️ [Event Storage] Error en callback para {event_type}: {e}")
    ...
    async def _notify_async_subscribers(self, event_type: str, data: Dict[str, Any]):
        ...
        for callback in self.async_subscribers[event_type]:
            try:
                ...
                print(f"⚠️ [Event Storage] Error en async callback para {event_type}: {e}")
...
class EventStorageMode:
    """
    Event Storage Mode - Sistema Event-Driven Multi-Agente RAG de Máxima Calidad para Empresas.
    ...
    ARQUITECTURA EVENT-DRIVEN:
    - Event Storage interno para comunicación entre componentes
    ...
    def __init__(
        ...
        if not config.openai_api_key:
            raise ValueError("OPENAI_API_KEY requerida para Event Storage Mode")
        ...
        # Enterprise Connectors - Detección automática de PDFs desde apps enterprise
        connector_config_dir = Path(config.audit_log_dir) / "connectors" if hasattr(config, "audit_log_dir") else None
        self.connector_manager = EnterpriseConnectorManager(
            config_dir=connector_config_dir,
            process_pdf_callback=self.process_pdf_from_connector
        )
        # Cargar conectores guardados
        self.connector_manager.load_all_connectors()

        # Raíz de almacenamiento para PDFs, texto y metadatos
        self.storage_root = Path(config.audit_log_dir) / "event_storage"
    ...
    def _publish_event(self, event_type: str, data: Dict[str, Any], async_notify: bool = True):
        """Publica en Event Storage y opcionalmente en Kafka."""
    ...
    def _on_document_uploaded(self, data: Dict[str, Any]):
        ...
        print(f"📄 [Event Storage] Documento subido detectado: {file_name}")
        ...
        try:
            ...
            print(f"⚠️ [Event Storage] Error en auto-procesamiento de documento: {e}")
    ...
    def _on_query_completed(self, data: Dict[str, Any]):
        ...
        print(f"✅ [Event Storage] Query completado: {query}...")
    ...
    def _on_error_occurred(self, data: Dict[str, Any]):
        ...
        print(f"❌ [Event Storage] Error en {context}: {error}")
    ...
    def _on_new_compliance_doc(self, data: Dict[str, Any]):
        ...
        print(f"🔍 [Event Storage] Nuevo documento de compliance detectado: {file_path}")
    ...
    def _on_document_updated(self, data: Dict[str, Any]):
        ...
        print(f"🔄 [Event Storage] Documento actualizado: {doc_id} desde {source} ({change_type})")
    ...
    def _on_compliance_complete(self, data: Dict[str, Any]):
        ...
        print(f"✅ [Event Storage] Compliance check completado: {entity_name} (Risk: {risk_score})")
    ...
    def _on_high_risk_detected(self, data: Dict[str, Any]):
        ...
        print(f"⚠️ [Event Storage] ALTO RIESGO detectado: {entity_name} (Risk: {risk_score})")
    ...
    def _on_query_executed(self, data: Dict[str, Any]):
        ...
        print(f"💬 [Event Storage] Query ejecutado: {query[:50]}...")
    ...
    def process_pdf_from_connector(self, file_url: str, metadata: Dict[str, Any]):
        """
        Función callback para procesar PDFs detectados por conectores enterprise.
        
        Esta función es llamada automáticamente cuando un conector detecta un PDF nuevo.
        
        Args:
            file_url: URL del archivo PDF
            metadata: Metadata del archivo (connector_type, file_id, file_name, etc.)
        """
        try:
            print(f"📥 [Event Storage] Procesando PDF desde {metadata.get('connector_type', 'unknown')}: {metadata.get('file_name', 'unknown')}")
            
            # Crear una sesión automática para PDFs de conectores (o usar una sesión global)
            connector_session_id = f"connector_{metadata.get('connector_id', 'default')}"
            
            # Si no existe la sesión, crearla
            if connector_session_id not in self.sessions:
                self.sessions[connector_session_id] = {
                    "history": [],
                    "docs": [],
                    "retriever": None,
                    "processed_files": set(),
            ...
            session = self.sessions[connector_session_id]
            
            # Descargar el archivo desde el conector
            connector = self.connector_manager.get_connector(metadata.get('connector_id', ''))
            if not connector:
                print(f"⚠️ [Event Storage] Conector no encontrado: {metadata.get('connector_id')}")
                return
            
            # Descargar archivo (async)
            asyncio.create_task(self._download_and_process_pdf(
                connector,
                file_url,
                metadata,
            ...
            # Publicar evento
            self._publish_event('pdf_detected_from_connector', {
            ...
            print(f"❌ [Event Storage] Error procesando PDF desde conector: {e}")
            self._publish_event('connector_pdf_error', {
                'error': str(e),
                'metadata': metadata
            })
    ...
    async def _download_and_process_pdf(
        ...
        """Descarga, procesa y persiste un PDF desde un conector."""
        try:
            file_id = metadata.get('file_id', '')
            
            # Descargar archivo
            content, file_metadata = await connector.download_file(file_url, file_id)
            
            # Crear objeto file-like para el processor
            file_obj = BytesIO(content)
            file_obj.name = metadata.get('file_name', 'document.pdf')
            ...
            # Agregar metadata de proveniencia
            ...
            # Actualizar retriever
            if session["docs"]:
                session["retriever"] = self.retriever_builder.build_hybrid_retriever(session["docs"])
            
            # Marcar como procesado
            session["processed_files"].add(file_id)
            
            # Persistir activos (PDF + texto + resumen + embeddings)
            await self._persist_document_assets(
                session_id=session_id,
            ...
            print(f"✅ [Event Storage] PDF procesado: {metadata.get('file_name')}")
            
            # Publicar evento
            self._publish_event('pdf_processed_from_connector', {
                'session_id': session_id,
                'file_name': metadata.get('file_name'),
            ...
            print(f"❌ [Event Storage] Error descargando/procesando PDF: {e}")
    ...
    async def _process_documents_async(
        ...
        try:
            print(f"📄 [Event Storage Mode] Procesando {len(new_files)} nuevos documentos en tiempo real...")
            ...
            for file_obj in new_files:
                try:
                    # Leer bytes del archivo (para persistencia y reuso)
                    raw_bytes = b""
                    file_name = getattr(file_obj, "name", getattr(file_obj, "filename", "document.pdf"))
                    if hasattr(file_obj, "read"):
                        raw_bytes = file_obj.read()
                    elif isinstance(file_obj, (str, Path)):
                        raw_bytes = Path(file_obj).read_bytes()
                    
                    # Recrear file-like para el processor
                    proc_file = BytesIO(raw_bytes) if raw_bytes else file_obj
                    proc_file.name = file_name
                    
                    # Procesar documento individual
                    file_docs = self.processor.process([proc_file])
                    session["docs"].extend(file_docs)
                    processed_count += 1
                    ...
                    print(f"⚠️ [Event Storage Mode] Error procesando archivo: {e}")
            ...
            print(f"✅ [Event Storage Mode] Procesamiento completado: {processed_count} documentos")
            
        except Exception as e:
            print(f"❌ [Event Storage Mode] Error en procesamiento asíncrono: {e}")
    ...
    async def _generate_realtime_insights(self, session_id: str, doc: Document):
        ...
        try:
            ...
            print(f"⚠️ [Event Storage Mode] Error generando insight: {e}")
    ...
    async def stream_realtime_insights(self, session_id: str) -> AsyncIterator[Dict[str, Any]]:
        ...
        Usage:
            async for insight in event_storage_mode.stream_realtime_insights(session_id):
                print(insight)
        """
        last_index = 0
    ...
    async def _stream_query_insights(
        ...
        try:
            ...
            print(f"⚠️ [Event Storage Mode] Error en streaming de insights: {e}")
    ...
    async def process_query_async(
        ...
        try:
            ...
            print(f"⚠️ [Event Storage Mode] Error agregando pasos de razonamiento: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Event Storage Mode] Error en Reinforcement Planning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Event Storage Mode] Error en Path-dependent Reasoning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Event Storage Mode] Error consultando MCP: {e}")
        ...
        try:
            ...
            formatted_answer += "- 📡 **Event Storage:** Arquitectura event-driven para procesamiento automático\n"
    ...
    async def _persist_document_assets(
        self,
        session_id: str,
        file_name: str,
        raw_bytes: bytes,
        ...
        """
        Persiste PDF, texto, resúmenes y embeddings en storage local (y deja listo para DB/Vector DB).
        """
        try:
            doc_id = metadata.get("file_id") or file_name
            folder = metadata.get("folder") or "default"

            # Rutas de almacenamiento
            doc_dir = self.storage_root / folder / doc_id
            doc_dir.mkdir(parents=True, exist_ok=True)

            # Guardar PDF
            if raw_bytes:
                pdf_path = doc_dir / file_name
                await asyncio.get_event_loop().run_in_executor(
                    None, pdf_path.write_bytes, raw_bytes
                )

            # Texto completo (concatenar chunks)
            full_text = "\n\n".join([d.page_content for d in docs])
            text_path = doc_dir / f"{file_name}.text.json"
            text_payload = {
                "file_name": file_name,
                "doc_id": doc_id,
                "session_id": session_id,
            ...
            await asyncio.get_event_loop().run_in_executor(
                None, text_path.write_text, json.dumps(text_payload, ensure_ascii=False, indent=2), "utf-8"
            )

            # Resúmenes (corto y largo)
            short_summary, long_summary = await self._summarize_text(full_text)
            summary_path = doc_dir / f"{file_name}.summary.json"
            summary_payload = {
            ...
            await asyncio.get_event_loop().run_in_executor(
                None, summary_path.write_text, json.dumps(summary_payload, ensure_ascii=False, indent=2), "utf-8"
            )

            # Embeddings (vector DB-ready): truncar texto largo para embed
            embed_text = full_text[:4000]  # evitar contextos enormes
            embeddings = self.retriever_builder.embeddings.embed_documents([embed_text])
            embeddings_path = doc_dir / f"{file_name}.embedding.json"
            await asyncio.get_event_loop().run_in_executor(
                None,
                embeddings_path.write_text,
                json.dumps({"embedding": embeddings[0], "model": self.config.embedding_model}, ensure_ascii=False),
                "utf-8",
            )

            # Metadata principal
            meta_path = doc_dir / f"{file_name}.meta.json"
            meta_payload = {
                "file_name": file_name,
                "doc_id": doc_id,
                "folder": folder,
                ...
                "pdf_path": str(doc_dir / file_name) if raw_bytes else None,
            }
            await asyncio.get_event_loop().run_in_executor(
                None, meta_path.write_text, json.dumps(meta_payload, ensure_ascii=False, indent=2), "utf-8"
            )
            ...
            print(f"⚠️ [Event Storage] Error persistiendo activos: {e}")
            self._publish_event(
                "document_persistence_error",
                {"session_id": session_id, "file_name": file_name, "error": str(e)},
            )

    async def _summarize_text(self, text: str) -> Tuple[str, str]:
        """Genera resumen corto y largo del texto."""
        try:
            if not text.strip():
                return "", ""
            prompt = (
                "Eres un analista empresarial. Resume el siguiente texto en dos niveles:\n"
                "1) Resumen corto (3 bullets)\n"
                "2) Resumen largo (1-2 párrafos)\n\n"
                "Texto:\n"
            )
            llm_response = await self.llm.apredict(
                prompt + text[:6000]  # limitar para evitar desbordes
            )
            # Heurística simple para separar
            parts = llm_response.split("\n\n", 1)
            short_summary = parts[0].strip()
            long_summary = parts[1].strip() if len(parts) > 1 else llm_response.strip()
            return short_summary, long_summary
        except Exception as e:
            print(f"⚠️ [Event Storage] No se pudo generar resumen: {e}")
            return "", ""
        ...
        try:
            ...
            for conn_id, connection in self.mcp_manager.connections.items():
                ...
                try:
                    ...
                    print(f"⚠️ [Event Storage Mode] Error consultando MCP {connection.name}: {e}")
                    continue
            ...
            print(f"⚠️ [Event Storage Mode] Error en consulta MCP: {e}")
            return None
    ...
    def handle_webhook_event(self, event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"📡 [Event Storage] Webhook recibido: {event_type}")
            ...
            print(f"❌ [Event Storage] Error procesando webhook: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    def sync_document_source(self, source: str, interval: int = 60) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"🔄 [Event Storage] Sincronizando desde {source} (intervalo: {interval}s)")
            ...
            print(f"❌ [Event Storage] Error en sync: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    def subscribe_to_external_webhook(self, source: str, callback_url: str) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"🔔 [Event Storage] Suscribiendo a webhooks de {source} -> {callback_url}")
            ...
            print(f"❌ [Event Storage] Error suscribiendo webhook: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    async def _process_query_parallel(
        ...
        Mantiene toda la funcionalidad específica de Event Storage Mode (persistencia, connectors, etc.).
        """
        ...
        def analyze_single_document(source_name: str, file_docs: List[Document]) -> Tuple[str, str]:
            """Analiza un solo documento con el prompt del usuario (Event Storage Mode)."""
            try:
                doc_content = "\n\n".join([doc.page_content for doc in file_docs])
                if len(doc_content) > 4000:
                    doc_content = doc_content[:4000] + "..."
                
                # Prompt adaptado para Event Storage Mode (persistencia y enterprise connectors)
                prompt = f"""Eres un analista estratégico de nivel C-Suite especializado en análisis de documentos enterprise con persistencia y almacenamiento optimizado. Tu tarea es analizar ESTE documento específico para responder DIRECTAMENTE la pregunta del usuario.
...
RESPUESTA ESPECÍFICA A LA PREGUNTA DEL USUARIO (300-500 palabras):"""
        ...
        print(f"🔄 [Event Storage Mode] Procesando {len(docs_by_source)} documentos en paralelo...")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"✅ [Event Storage Mode] Análisis completado para: {Path(doc_name).name}")
                    ...
                    print(f"❌ [Event Storage Mode] Error procesando {source_name}: {e}")
        ...
        synthesis_prompt = f"""Eres un consultor estratégico senior de nivel C-Suite especializado en análisis de documentos enterprise con persistencia y almacenamiento optimizado. Has analizado {len(individual_analyses)} documentos individualmente, cada uno respondiendo la pregunta del usuario.
...
# Instancia global
_event_storage_mode_instance: Optional[EventStorageMode] = None


def get_event_storage_mode(
    config: AppConfig,
    processor: DocumentProcessor,
    retriever_builder: RetrieverBuilder,
    context_manager: Optional[Any] = None
) -> EventStorageMode:
    """Obtiene o crea la instancia global de Event Storage Mode."""
    global _event_storage_mode_instance
    
    if _event_storage_mode_instance is None:
        _event_storage_mode_instance = EventStorageMode(
            config=config,
            processor=processor,
    ...
    return _event_storage_mode_instance


def run_event_storage_mode(
    message: str,
    history: List[Tuple[str, str]],
    files: List[Any],
    session_id: str,
    ...
    """
    Función principal para ejecutar Event Storage Mode.
    ...
    # Obtener instancia
    event_storage_mode = get_event_storage_mode(
        config=config,
        processor=processor,
        retriever_builder=retriever_builder,
        context_manager=context_manager
    ...
    if files:
        result = event_storage_mode.process_documents(session_id, files)
    ...
    try:
        ...
        new_history, error, metadata = loop.run_until_complete(
            event_storage_mode.process_query_async(
                session_id=session_id,
                message=message,
                history=history,
                speed_mode=speed_mode,
