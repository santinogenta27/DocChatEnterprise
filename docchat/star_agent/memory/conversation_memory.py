...
class ConversationContext:
    """Contexto acumulado de una conversaciÃ³n."""
    session_id: str
    user_id: str
    ...
    customer_dna: Optional[Dict[str, Any]] = None  # "Ficha mÃ©dica" del comprador (NIVEL DIOS)
    last_updated: str = field(default_factory=lambda: datetime.now().isoformat())


@dataclass
class ConversationSummary:
    """Resumen inteligente de una conversaciÃ³n."""
    session_id: str
    user_id: str
    summary: str
    key_points: List[str]
...
class ConversationMemory:
    ...
    def __init__(self, storage_dir: Path):
        ...
        self.max_long_term_summaries = 10  # MÃ¡ximo de resÃºmenes por usuario
    
    def add_message(self, session_id: str, user_id: str, role: str, content: str, metadata: Optional[Dict] = None):
        """
        AÃ±ade un mensaje a la memoria de la conversaciÃ³n.
        ...
        # Guardar memoria a largo plazo periÃ³dicamente
    ...
    def _update_context_from_message(self, session_id: str, role: str, content: str, metadata: Optional[Dict]):
        """Actualiza el contexto acumulado basÃ¡ndose en el mensaje."""
        ...
        if role == "user":
            ...
            # Timeline
            if any(word in content_lower for word in ["urgente", "rÃ¡pido", "pronto", "inmediato", "fecha", "cuÃ¡ndo"]):
        ...
        # Generar/actualizar customer_dna si hay suficiente informaciÃ³n (cada 7 mensajes)
    ...
    def get_conversation_context(self, session_id: str) -> Optional[ConversationContext]:
        """Obtiene el contexto acumulado de una conversaciÃ³n."""
        return self._conversation_contexts.get(session_id)
    
    def get_accumulated_context(self, session_id: str) -> ConversationContext:
        """Obtiene el contexto acumulado de una conversaciÃ³n (alias para compatibilidad)."""
        if session_id not in self._conversation_contexts:
            # Crear contexto vacÃ­o si no existe
            user_id = "unknown"
    ...
    def get_conversation_summary_text(self, session_id: str) -> str:
        """
        Genera un resumen de texto del contexto de conversaciÃ³n para usar en prompts.
        ...
        if context.needs_mentioned:
            summary_parts.append(f"**Necesidades mencionadas:** {', '.join(context.needs_mentioned[-3:])}")  # Ãšltimas 3
        ...
        if context.products_interested:
            summary_parts.append(f"**Productos de interÃ©s:** {', '.join(context.products_interested)}")
    ...
    def get_recent_history_text(self, session_id: str, last_n: int = 5) -> str:
        """
        Obtiene los Ãºltimos N mensajes como texto.
        
        Args:
            session_id: ID de la sesiÃ³n
            last_n: NÃºmero de mensajes recientes
    ...
    def get_long_term_memory_text(self, user_id: str) -> str:
        ...
        summaries = self._user_long_term_memory[user_id][-3:]  # Ãšltimas 3 conversaciones
        
        memory_parts = []
        for summary in summaries:
            memory_parts.append(f"**ConversaciÃ³n anterior ({summary.created_at[:10]}):**")
    ...
    def _create_conversation_summary(self, session_id: str):
        """Crea un resumen inteligente de la conversaciÃ³n."""
        context = self._conversation_contexts.get(session_id)
        history = self._conversation_histories.get(session_id)
        ...
        # Crear resumen bÃ¡sico (puede mejorarse con LLM)
        summary_text = f"ConversaciÃ³n sobre {', '.join(context.needs_mentioned[:3]) if context.needs_mentioned else 'productos y servicios'}"
        
        key_points = []
        if context.products_interested:
            key_points.append(f"InterÃ©s en productos: {', '.join(context.products_interested)}")
        if context.objections_raised:
    ...
    def _load_user_long_term_memory(self, user_id: str):
        ...
        try:
            ...
            print(f"âš ï¸ Error cargando memoria a largo plazo para {user_id}: {e}")
    ...
    def _save_user_long_term_memory(self, user_id: str):
        ...
        try:
            ...
            print(f"âš ï¸ Error guardando memoria a largo plazo para {user_id}: {e}")
    
    def save_session_memory(self, session_id: str):
        """Guarda la memoria de una sesiÃ³n completa."""
        context = self._conversation_contexts.get(session_id)
        if context:
            # Crear resumen final
            self._create_conversation_summary(session_id)
    ...
    def get_full_context_for_prompt(self, session_id: str, user_id: str, include_history: bool = True, include_long_term: bool = True) -> str:
        ...
        # Contexto acumulado de esta sesiÃ³n
        context_summary = self.get_conversation_summary_text(session_id)
        if context_summary:
            context_parts.append("**CONTEXTO ACUMULADO DE ESTA CONVERSACIÃ“N:**")
    ...
    def _generate_customer_dna(self, session_id: str):
        ...
        Esto es como una "ficha mÃ©dica" que resume la personalidad, preferencias y estilo
        ...
        if not context or not history or len(history) < 3:
            return  # No hay suficiente informaciÃ³n aÃºn
        ...
        try:
            ...
            print(f"ðŸ§¬ Customer DNA generado para sesiÃ³n {session_id[:8]}: {customer_dna.get('summary', 'N/A')[:50]}...")
            
        except Exception as e:
            print(f"âš ï¸ Error generando customer_dna: {e}")
            # Si falla, crear DNA bÃ¡sico
            context.customer_dna = {
                "summary": "Cliente en proceso de evaluaciÃ³n",
                "sensibility": "unknown",
                "communication_style": "unknown"
            }
    
    def _generate_dna_from_rules(self, context: ConversationContext, recent_messages: List[Dict]) -> Dict[str, Any]:
        """Genera customer_dna usando reglas bÃ¡sicas (fallback sin LLM)."""
        dna = {
            "summary": "",
            "sensibility": "both",
        ...
        if dna["communication_style"] == "direct":
            traits.append("ComunicaciÃ³n directa")
        
        dna["personality_traits"] = traits
        
        dna["summary"] = f"Cliente {dna['sensibility']}-orientado con estilo {dna['communication_style']}. " + \
                        (f"ObjeciÃ³n recurrente: {dna['recurring_objection']}" if dna["recurring_objection"] else "")
