...
Rastrea eventos crÃ­ticos: add_to_cart, checkout, purchase_complete
IntegraciÃ³n con Google Analytics, Meta Pixel, y tracking interno
"""
...
class ConversionEvent(Enum):
    """Eventos de conversiÃ³n rastreables."""
...
class ConversionEventData:
    """Datos de un evento de conversiÃ³n."""
...
class ConversionTracker:
    ...
    CaracterÃ­sticas:
    - Tracking de eventos crÃ­ticos (add_to_cart, checkout, purchase)
    - IntegraciÃ³n con Google Analytics, Meta Pixel
    ...
    def __init__(self, storage_dir: Path, enable_ga: bool = False, enable_meta_pixel: bool = False):
        ...
        # Cargar configuraciÃ³n si estÃ¡ habilitada
        if enable_ga:
            import os
    ...
    def track_event(self,
        ...
        """
        Rastrea un evento de conversiÃ³n.
        ...
        # Escribir si el buffer estÃ¡ lleno
        ...
        # Log
        print(f"ðŸ“Š [Conversion Tracking] {event_type.value}: {product_name or 'N/A'} | Value: {value} {currency} | Session: {session_id[:8]}")
        
        return event_data
    
    def track_product_viewed(self, session_id: str, user_id: str, product_id: str, product_name: str, price: float = 0.0):
        """Rastrea visualizaciÃ³n de producto."""
    ...
    def track_add_to_cart(self, session_id: str, user_id: str, product_id: str, product_name: str, price: float, quantity: int = 1):
        """Rastrea producto aÃ±adido al carrito."""
        return self.track_event(
    ...
    def _flush_buffer(self):
        ...
        try:
            ...
            print(f"âš ï¸ Error escribiendo eventos de conversiÃ³n: {e}")
        
        # Limpiar buffer
        self._events_buffer.clear()
    
    def _send_to_external_services(self, event_data: ConversionEventData):
        """EnvÃ­a eventos a servicios externos (GA, Meta Pixel)."""
        # Google Analytics
    ...
    def _send_to_google_analytics(self, event_data: ConversionEventData):
        """EnvÃ­a evento a Google Analytics (Measurement Protocol)."""
        try:
            import requests
            ...
            # Nota: En producciÃ³n, esto debe hacerse de forma asÃ­ncrona
            # requests.post(ga_endpoint, json=payload, timeout=2)
            
        except Exception as e:
            print(f"âš ï¸ Error enviando a Google Analytics: {e}")
    
    def _send_to_meta_pixel(self, event_data: ConversionEventData):
        """EnvÃ­a evento a Meta Pixel (Conversions API)."""
        try:
            import requests
            ...
            # Nota: Requiere access_token, debe hacerse de forma asÃ­ncrona en producciÃ³n
            # requests.post(meta_endpoint, json=payload, headers={"Authorization": f"Bearer {access_token}"}, timeout=2)
            
        except Exception as e:
            print(f"âš ï¸ Error enviando a Meta Pixel: {e}")
    ...
    def _load_events_period(self, start_date: Optional[datetime], end_date: Optional[datetime]) -> List[ConversionEventData]:
        """Carga eventos del perÃ­odo especificado."""
        events = []
        ...
        for events_file in self.storage_dir.glob("conversion_events_*.jsonl"):
            try:
                ...
                print(f"âš ï¸ Error cargando eventos de {events_file}: {e}")
