"""Procesador de documentos usando PyPDF2 para STAR AGENT RAG.
...
# Constantes
CACHE_EXPIRE_DAYS = 30  # Cache vÃ¡lido por 30 dÃ­as
MAX_FILE_SIZE_MB = 100
MAX_TOTAL_SIZE_MB = 500

# ConfiguraciÃ³n de chunking (como Meta: fragmentaciÃ³n inteligente por unidades de significado)
...
class DocumentProcessor:
    ...
    def __init__(self, cache_dir: Optional[Path] = None):
        ...
        if not LANGCHAIN_AVAILABLE:
            raise ImportError(
                "LangChain no estÃ¡ instalado. Instala con: pip install langchain langchain-core langchain-text-splitters"
            )
        
        self.cache_dir = cache_dir or Path("docchat/star_agent/rag/cache")
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        
        # Inicializar text splitter (chunking inteligente por unidades de significado)
        # Meta usa fragmentaciÃ³n por unidades de significado, no por pÃ¡ginas
        self.splitter = RecursiveCharacterTextSplitter(
            chunk_size=CHUNK_SIZE,
            chunk_overlap=CHUNK_OVERLAP,
            length_function=len,
            separators=["\n\n", "\n", ". ", " ", ""]  # Priorizar pÃ¡rrafos completos
        )
    
    def validate_files(self, files: List[Any]) -> None:
        """Valida que los archivos no excedan el tamaÃ±o mÃ¡ximo.
        
        Args:
            files: Lista de archivos a validar
            
        Raises:
            ValueError: Si el tamaÃ±o total excede el lÃ­mite
        """
        total_size = 0
        for file in files:
            ...
            if path.exists():
                ...
                if size_mb > MAX_FILE_SIZE_MB:
                    raise ValueError(
                        f"Archivo {path.name} excede el tamaÃ±o mÃ¡ximo de {MAX_FILE_SIZE_MB}MB"
                    )
        
        if total_size > MAX_TOTAL_SIZE_MB:
            raise ValueError(
                f"TamaÃ±o total de archivos ({total_size:.2f}MB) excede el lÃ­mite de {MAX_TOTAL_SIZE_MB}MB"
            )
    ...
    def process(self, files: List[Any]) -> List[Document]:
        ...
        Returns:
            Lista de documentos chunked (sin embeddings aÃºn)
        """
        if not files:
            return []
        ...
        for file in files:
            try:
                ...
                if cache_path.exists() and self._is_cache_valid(cache_path):
                    ...
                    print(f"âœ… Cargado desde cache: {file_path.name} ({len(chunks)} chunks)")
                else:
                    ...
                    if chunks:
                        ...
                        print(f"âœ… Procesado: {file_path.name} ({len(chunks)} chunks)")
        ...
        print(f"\nâœ… Total: {len(all_chunks)} chunks Ãºnicos procesados (SIN llamadas a API)")
        return all_chunks
    ...
    def _process_file(self, file_path: Path, file_hash: str) -> List[Document]:
        ...
        if ext not in [".pdf", ".txt", ".md"]:
            print(f"âš ï¸ Formato no soportado: {ext} (solo PDF, TXT, MD)")
            return []
    ...
    def _process_pdf_pypdf2_only(self, file_path: Path, file_hash: str) -> List[Document]:
        """Procesar PDF usando SOLO PyPDF2 (rÃ¡pido, sin APIs, sin Docling)."""
        import time
        
        file_size_mb = file_path.stat().st_size / (1024 * 1024)
        print(f"   ðŸ“„ Procesando PDF con PyPDF2: {file_path.name} ({file_size_mb:.2f} MB)")
            ...
            with open(file_path, "rb") as f:
                ...
                for page_num, page in enumerate(pdf_reader.pages, 1):
                    try:
                        ...
                        if text.strip():
                            # FragmentaciÃ³n inteligente: mantener pÃ¡rrafos completos
                            text_parts.append(text.strip())
                        # Mostrar progreso cada 10 pÃ¡ginas
                        if page_num % 10 == 0 or page_num == total_pages:
                            print(f"      ðŸ“„ PÃ¡gina {page_num}/{total_pages} procesada...", end='\r')
                    except Exception as e:
                        # Continuar con siguiente pÃ¡gina si una falla
                        print(f"      âš ï¸ Error en pÃ¡gina {page_num}: {e}")
                        continue
                
                print()  # Nueva lÃ­nea despuÃ©s del progreso
                
                if not text_parts:
                    print(f"   âš ï¸ No se extrajo texto del PDF")
                    return []
                
                # Combinar todo el texto (fragmentaciÃ³n inteligente como Meta)
                full_text = "\n\n".join(text_parts)
                
                # Dividir en chunks usando text splitter (unidades de significado)
                # Meta divide por unidades de significado, no por pÃ¡ginas
                ...
                print(f"   âœ… PyPDF2 completado en {elapsed:.1f}s â†’ {len(documents)} chunks (fragmentaciÃ³n inteligente)")
                
                return documents
                
        except ImportError:
            print(f"   âŒ PyPDF2 no estÃ¡ instalado. Instala con: pip install PyPDF2")
            return []
        except Exception as e:
            error_msg = str(e).lower()
            if "pycryptodome" in error_msg or "aes" in error_msg or "encrypted" in error_msg:
                print(f"   âŒ PDF encriptado detectado. PyPDF2 requiere PyCryptodome para desencriptar.")
                print(f"   ðŸ’¡ Instala con: pip install pycryptodome")
            else:
                print(f"   âŒ Error procesando PDF: {str(e)[:100]}")
            return []
    ...
    def _process_text_file(self, file_path: Path, file_hash: str) -> List[Document]:
        ...
        try:
            ...
            print(f"   âœ… Archivo de texto procesado: {len(documents)} chunks")
            return documents
            
        except Exception as e:
            print(f"   âŒ Error procesando archivo de texto: {e}")
            return []
