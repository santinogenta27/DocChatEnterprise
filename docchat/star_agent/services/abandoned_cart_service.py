"""
Abandoned Cart Service - Servicio de RecuperaciÃ³n de Carritos Abandonados
Maneja recordatorios automÃ¡ticos y recuperaciÃ³n de carritos abandonados
"""
...
class AbandonedCartService:
    """
    Servicio de recuperaciÃ³n de carritos abandonados.
    
    CaracterÃ­sticas:
    - DetecciÃ³n automÃ¡tica de carritos abandonados
    - Recordatorios programados inteligentes
    - IntegraciÃ³n con webhooks de Shopify/WooCommerce
    """
    ...
    def __init__(self, storage_dir: Path):
        ...
        # ConfiguraciÃ³n
        self.abandonment_threshold_minutes = 5  # Considerar abandonado despuÃ©s de 5 minutos
    ...
    def get_abandoned_carts_for_reminder(self) -> List[AbandonedCart]:
        ...
        for cart in self._abandoned_carts.values():
            ...
            # Determinar si necesita recordatorio segÃºn intervalos
    ...
    def mark_reminder_sent(self, cart: AbandonedCart):
        """Marca que se enviÃ³ un recordatorio."""
    ...
    def generate_reminder_message(self, cart: AbandonedCart) -> str:
        ...
        if items_count > 3:
            items_description += f" y {items_count - 3} mÃ¡s"
        
        # Mensaje personalizado segÃºn nÃºmero de recordatorios
        if cart.reminders_sent == 0:
            message = (
                f"Hola! ðŸ‘‹ Vi que dejaste {items_count} producto(s) en tu carrito: {items_description}. "
                f"Â¿Tienes alguna pregunta o necesitas ayuda con algo? Estoy aquÃ­ para ayudarte."
            )
        elif cart.reminders_sent == 1:
            message = (
                f"Recordatorio amigable: Tu carrito con {items_count} producto(s) sigue esperÃ¡ndote. "
                f"Â¿Hay algo especÃ­fico en lo que pueda ayudarte a decidir?"
            )
        else:
            # Ãšltimo recordatorio, mÃ¡s directo
            message = (
                f"Ãšltima oportunidad: Tu carrito con {items_count} producto(s) sigue disponible. "
                f"Â¿Te gustarÃ­a continuar con tu compra o tienes alguna duda que pueda resolver?"
            )
    ...
    def handle_webhook_event(self, event_data: Dict[str, Any]) -> Optional[AbandonedCart]:
        ...
        Returns:
            AbandonedCart si se detectÃ³ abandono, None si no
        """
    ...
    def _save_cart(self, cart: AbandonedCart):
        ...
        try:
            ...
            print(f"âš ï¸ Error guardando carrito abandonado: {e}")
    ...
    def _load_cart(self, user_id: str, session_id: str) -> Optional[AbandonedCart]:
        ...
        try:
            ...
            print(f"âš ï¸ Error cargando carrito: {e}")
            return None
