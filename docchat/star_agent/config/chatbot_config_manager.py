"""Gestor de configuraciÃ³n del chatbot STAR AGENT.

Permite cargar y guardar configuraciones del chatbot desde JSON (preferido) o .env (fallback).
JSON es mÃ¡s fÃ¡cil de usar para usuarios no tÃ©cnicos.
"""
...
@dataclass
class ChatbotConfig:
    """ConfiguraciÃ³n completa del chatbot."""
    ...
    # Agendamiento de Citas (Booking/CTA) - PRIORIDAD ALTA ðŸš¨
    booking_enabled: bool = False
    calendly_url: str = ""
    google_calendar_url: str = ""
    crm_webhook_url: str = ""
    crm_type: str = ""  # hubspot, salesforce, pipedrive, ""
    booking_message: str = "Veo que estÃ¡s listo para empezar. Â¿Te parece bien agendar una demo? Puedes elegir el horario que mejor te convenga."
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ChatbotConfig":
        """Crea configuraciÃ³n desde diccionario."""
        return cls(**data)


class ChatbotConfigManager:
    """Gestor de configuraciÃ³n del chatbot.
    
    Usa JSON como fuente principal (mÃ¡s fÃ¡cil para usuarios no tÃ©cnicos).
    ...
    def __init__(self, config_json_path: Optional[Path] = None, env_path: Optional[Path] = None):
        # JSON es la fuente principal (mÃ¡s fÃ¡cil para usuarios)
        self.config_json_path = config_json_path or Path("docchat/star_agent/config/chatbot_config.json")
        # .env como fallback
        self.env_path = env_path or Path(".env")
        self.config: Optional[ChatbotConfig] = None
    
    def load(self) -> ChatbotConfig:
        """Carga configuraciÃ³n desde JSON (preferido) o .env (fallback)."""
        # Intentar cargar desde JSON primero
        if self.config_json_path.exists():
            try:
                return self.load_from_json()
            except Exception as e:
                print(f"âš ï¸ Error cargando config desde JSON: {e}. Intentando .env...")
    ...
    def save_to_json(self, chatbot_config: ChatbotConfig) -> bool:
        ...
        try:
            ...
            self.config = chatbot_config
            print(f"âœ… ConfiguraciÃ³n guardada en: {self.config_json_path}")
            return True
        except Exception as e:
            print(f"âŒ Error guardando JSON: {e}")
            return False
    
    def load_from_env(self) -> ChatbotConfig:
        """Carga configuraciÃ³n desde variables de entorno (.env) - Fallback."""
        ...
        chatbot_config.booking_message = os.getenv("DOCCHAT_CHATBOT_BOOKING_MESSAGE", "Veo que estÃ¡s listo para empezar. Â¿Te parece bien agendar una demo? Puedes elegir el horario que mejor te convenga.")
        
        self.config = chatbot_config
        return chatbot_config
    
    def save_to_env(self, chatbot_config: ChatbotConfig) -> bool:
        """Guarda configuraciÃ³n en .env - Solo para compatibilidad."""
        try:
            # Leer .env actual
            env_vars = {}
            ...
            print(f"âš ï¸ Error guardando .env (no crÃ­tico): {e}")
            return False
    
    def get_config(self) -> ChatbotConfig:
        """Obtiene la configuraciÃ³n actual (carga si no estÃ¡ cargada)."""
        if self.config is None:
            return self.load()
        return self.config
    
    def save(self, chatbot_config: ChatbotConfig, also_update_env: bool = True) -> bool:
        """Guarda configuraciÃ³n en JSON (y opcionalmente en .env para compatibilidad)."""
        # Guardar en JSON (principal)
        success = self.save_to_json(chatbot_config)
        
        # Opcionalmente tambiÃ©n actualizar .env para compatibilidad
        if also_update_env and success:
            try:
                self.save_to_env(chatbot_config)
            except Exception as e:
                print(f"âš ï¸ No se pudo actualizar .env (no crÃ­tico): {e}")
