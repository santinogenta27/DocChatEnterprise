...
- `star_agent_mode.py`: modo principal (`StarAgentMode`)
- `agents/star_agent_agent.py`: orquestador de ventas + soporte
...
config = load_config()
mode = StarAgentMode(config=config)
...
app = FastAPI()
mode = StarAgentMode(config=load_config())
