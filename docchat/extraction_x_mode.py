"""
Extraction X Mode - Sistema de Extracción Inteligente + Recomendaciones Accionables

SISTEMA DE EXTRACCIÓN INTELIGENTE:
- 📄 Procesa múltiples tipos de documentos: PDFs, contratos, emails, documentos legales, políticas, reportes, facturas, presentaciones
- 🔍 Extrae entidades: personas, empresas, organizaciones, productos, servicios
- 📅 Detecta fechas críticas: vencimientos, plazos, expiraciones con cálculo de días restantes
- ⚠️ Identifica riesgos: legales, financieros, operacionales, de compliance (alto/medio/bajo)
- 📋 Genera obligaciones: contratos, regulaciones, compromisos, requisitos
- 📊 Extrae métricas y valores numéricos importantes
- 👥 Identifica roles y responsabilidades
- 📑 Detecta cláusulas críticas y términos importantes

RECOMENDACIONES ACCIONABLES:
- 🚨 Recomendaciones operativas: vencimientos expirados/urgentes, renovaciones necesarias
- 🔴 Recomendaciones de riesgo: acciones inmediatas para riesgos altos/medios
- 🔒 Recomendaciones de compliance: verificación GDPR, PCI-DSS, etc.
- 🔄 Recomendaciones de workflow: asignaciones, notificaciones, flujos automáticos
- 📝 Resúmenes accionables: qué hacer, qué falta, qué riesgo hay, qué pasos seguir

ARQUITECTURA EVENT-DRIVEN:
- 📡 Extraction X Bus: mensajería interna para comunicación entre componentes
...
class SimpleEventBus:
    """Extraction X optimizado para real-time streaming y procesamiento asíncrono."""
    ...
    def publish(self, event_type: str, data: Dict[str, Any], async_notify: bool = True):
        ...
        for callback in self.subscribers[event_type]:
            try:
                ...
                print(f"⚠️ [Extraction X] Error en callback para {event_type}: {e}")
    ...
    async def _notify_async_subscribers(self, event_type: str, data: Dict[str, Any]):
        ...
        for callback in self.async_subscribers[event_type]:
            try:
                ...
                print(f"⚠️ [Extraction X] Error en async callback para {event_type}: {e}")
...
class ExtractionXMode:
    """
    Extraction X Mode - Sistema Event-Driven Multi-Agente RAG de Máxima Calidad para Empresas.
    ...
    ARQUITECTURA EVENT-DRIVEN:
    - Extraction X interno para comunicación entre componentes
    ...
    def __init__(
        ...
        if not config.openai_api_key:
            raise ValueError("OPENAI_API_KEY requerida para Extraction X Mode")
    ...
    def _publish_event(self, event_type: str, data: Dict[str, Any], async_notify: bool = True):
        """Publica en Extraction X y opcionalmente en Kafka."""
    ...
    def _on_document_uploaded(self, data: Dict[str, Any]):
        ...
        print(f"📄 [Extraction X] Documento subido detectado: {file_name}")
        ...
        try:
            ...
            print(f"⚠️ [Extraction X] Error en auto-procesamiento de documento: {e}")
    ...
    def _on_query_completed(self, data: Dict[str, Any]):
        ...
        print(f"✅ [Extraction X] Query completado: {query}...")
    ...
    def _on_error_occurred(self, data: Dict[str, Any]):
        ...
        print(f"❌ [Extraction X] Error en {context}: {error}")
    ...
    def _on_new_compliance_doc(self, data: Dict[str, Any]):
        ...
        print(f"🔍 [Extraction X] Nuevo documento de compliance detectado: {file_path}")
    ...
    def _on_document_updated(self, data: Dict[str, Any]):
        ...
        print(f"🔄 [Extraction X] Documento actualizado: {doc_id} desde {source} ({change_type})")
    ...
    def _on_compliance_complete(self, data: Dict[str, Any]):
        ...
        print(f"✅ [Extraction X] Compliance check completado: {entity_name} (Risk: {risk_score})")
    ...
    def _on_high_risk_detected(self, data: Dict[str, Any]):
        ...
        print(f"⚠️ [Extraction X] ALTO RIESGO detectado: {entity_name} (Risk: {risk_score})")
    ...
    def _on_query_executed(self, data: Dict[str, Any]):
        ...
        print(f"💬 [Extraction X] Query ejecutado: {query[:50]}...")
    ...
    async def _process_documents_async(
        ...
        try:
            print(f"📄 [Extraction X Mode] Procesando {len(new_files)} nuevos documentos en tiempo real...")
            ...
            for file_obj in new_files:
                try:
                    ...
                    # EXTRACCIÓN INTELIGENTE - Extraer entidades, riesgos, obligaciones, fechas críticas
                    extraction_result = await self._extract_intelligent_insights(
                        file_docs=file_docs,
                        file_obj=file_obj,
                        session_id=session_id
                    )
                    
                    # Guardar resultado de extracción en sesión
                    if "extractions" not in session:
                        session["extractions"] = []
                    session["extractions"].append(extraction_result)
                    
                    # Publicar evento de extracción completada
                    self._publish_event('intelligent_extraction_completed', {
                        'session_id': session_id,
                    ...
                    print(f"⚠️ [Extraction X Mode] Error procesando archivo: {e}")
            ...
            print(f"✅ [Extraction X Mode] Procesamiento completado: {processed_count} documentos")
            
        except Exception as e:
            print(f"❌ [Extraction X Mode] Error en procesamiento asíncrono: {e}")
    ...
    async def _generate_realtime_insights(self, session_id: str, doc: Document):
        ...
        try:
            ...
            print(f"⚠️ [Extraction X Mode] Error generando insight: {e}")
    
    async def _extract_intelligent_insights(
        self,
        ...
        """
        Extracción inteligente de información empresarial:
        - Detecta tipo de documento
        - Extrae entidades, fechas críticas, riesgos, obligaciones
        ...
        try:
            ...
            doc_type = await self._detect_document_type(full_text, file_name, file_extension)
            
            # 2. EXTRAER INFORMACIÓN ESTRUCTURADA
            extraction_prompt = f"""Analiza este documento empresarial y extrae TODA la información crítica en formato JSON.

TIPO DE DOCUMENTO: {doc_type}

INSTRUCCIONES:
1. Extrae TODAS las entidades (personas, empresas, organizaciones, productos, servicios)
2. Identifica TODAS las fechas críticas (vencimientos, plazos, fechas límite, expiraciones)
3. Detecta TODOS los riesgos (legales, financieros, operacionales, de compliance)
4. Identifica TODAS las obligaciones (contratos, regulaciones, compromisos, requisitos)
5. Extrae métricas y valores numéricos importantes
6. Identifica roles y responsabilidades
7. Detecta cláusulas críticas o términos importantes
...
{{
    "entities": [
        {{"type": "persona|empresa|organizacion|producto|servicio", "name": "...", "context": "..."}}
    ],
    "critical_dates": [
        {{"date": "YYYY-MM-DD", "type": "vencimiento|plazo|expiracion|limite", "description": "...", "days_until": N}}
    ],
    "risks": [
        {{"level": "alto|medio|bajo", "type": "legal|financiero|operacional|compliance", "description": "...", "impact": "..."}}
    ],
    "obligations": [
        {{"type": "contrato|regulacion|compromiso|requisito", "description": "...", "responsible": "...", "deadline": "..."}}
    ],
    "metrics": [
    ...
    "critical_clauses": [
        {{"clause": "...", "importance": "alta|media|baja", "description": "..."}}
    ]
}}"""
            ...
            try:
                ...
                if json_match:
                    ...
                    extracted_data = {"error": "No se pudo parsear JSON", "raw": extraction_text}
            ...
            # 4. CALCULAR DÍAS HASTA VENCIMIENTOS
            for date_item in extracted_data.get("critical_dates", []):
                if "date" in date_item:
                    try:
                        date_obj = datetime.fromisoformat(date_item["date"].replace("Z", "+00:00"))
                        days_until = (date_obj - datetime.now()).days
                        date_item["days_until"] = days_until
                        date_item["is_urgent"] = days_until <= 30
                        date_item["is_expired"] = days_until < 0
                    except:
                        pass
            ...
            result = {
                ...
                "summary": {
                    "total_entities": len(extracted_data.get("entities", [])),
                    "total_dates": len(extracted_data.get("critical_dates", [])),
                    "total_risks": len(extracted_data.get("risks", [])),
                    "high_risk_count": len([r for r in extracted_data.get("risks", []) if r.get("level") == "alto"]),
                    "total_obligations": len(extracted_data.get("obligations", [])),
                    "urgent_dates": len([d for d in extracted_data.get("critical_dates", []) if d.get("is_urgent", False)]),
                    "expired_dates": len([d for d in extracted_data.get("critical_dates", []) if d.get("is_expired", False)])
                }
            }
            
            print(f"✅ [Extraction X] Extracción inteligente completada: {file_name}")
            print(f"   - Entidades: {result['summary']['total_entities']}")
            print(f"   - Fechas críticas: {result['summary']['total_dates']} ({result['summary']['urgent_dates']} urgentes)")
            print(f"   - Riesgos: {result['summary']['total_risks']} ({result['summary']['high_risk_count']} altos)")
            print(f"   - Obligaciones: {result['summary']['total_obligations']}")
            print(f"   - Recomendaciones: {len(recommendations)}")
            
            return result
            
        except Exception as e:
            print(f"⚠️ [Extraction X] Error en extracción inteligente: {e}")
            return {
                "file_name": getattr(file_obj, "name", "unknown"),
                "error": str(e),
                "extraction_timestamp": datetime.now().isoformat()
    ...
    async def _detect_document_type(self, text: str, file_name: str, file_extension: str) -> str:
        """Detecta el tipo de documento basado en contenido y nombre."""
        file_lower = file_name.lower()
        text_lower = text.lower()[:2000]  # Primeros 2000 chars para análisis rápido
        
        # Detección por palabras clave
        if any(word in file_lower for word in ["contrato", "contract", "acuerdo", "agreement"]):
            return "contrato"
        elif any(word in file_lower for word in ["factura", "invoice", "bill"]):
            return "factura"
        elif any(word in file_lower for word in ["política", "policy", "norma", "regulación"]):
            return "política"
        elif any(word in file_lower for word in ["reporte", "report", "informe"]):
            return "reporte"
        elif any(word in file_lower for word in ["email", "correo", "mail"]):
            return "email"
        elif any(word in file_lower for word in ["legal", "jurídico", "ley", "law"]):
            return "documento_legal"
        elif any(word in file_lower for word in ["presentación", "presentation", "ppt", "slides"]):
            return "presentación"
        elif any(word in text_lower for word in ["cláusula", "clause", "artículo", "article", "sección", "section"]):
            return "documento_legal"
        elif any(word in text_lower for word in ["vencimiento", "expira", "expires", "deadline", "plazo"]):
            return "contrato"
        elif any(word in text_lower for word in ["póliza", "poliza", "insurance", "seguro"]):
            return "póliza"
        else:
    ...
    async def _generate_actionable_recommendations(
        ...
        """Genera recomendaciones accionables basadas en la extracción."""
        recommendations = []
        
        # 1. RECOMENDACIONES OPERATIVAS (fechas críticas)
        for date_item in extracted_data.get("critical_dates", []):
            days_until = date_item.get("days_until", 999)
            ...
            if days_until < 0:
                recommendations.append({
                    "type": "operativa",
                    "priority": "alta",
                    "category": "vencimiento_expirado",
                    "title": f"⚠️ VENCIMIENTO EXPIRADO: {description}",
                    "action": f"Renovar o actualizar inmediatamente. El plazo venció hace {abs(days_until)} días.",
                    "assign_to": "Gerente de Contratos",
                    "deadline": "Inmediato"
                })
            elif days_until <= 7:
                recommendations.append({
                    "type": "operativa",
                    "priority": "alta",
                    "category": "vencimiento_urgente",
                    "title": f"🚨 VENCIMIENTO URGENTE: {description}",
                    "action": f"Renovar o tomar acción inmediata. Vence en {days_until} días.",
                    "assign_to": "Gerente de Contratos",
                    "deadline": f"{days_until} días"
                })
            elif days_until <= 30:
                recommendations.append({
                    "type": "operativa",
                    "priority": "media",
                    "category": "vencimiento_proximo",
                    "title": f"📅 VENCIMIENTO PRÓXIMO: {description}",
                    "action": f"Planificar renovación. Vence en {days_until} días.",
                    "assign_to": "Equipo Legal",
                    "deadline": f"{days_until} días"
                })
        
        # 2. RECOMENDACIONES DE RIESGO
        for risk in extracted_data.get("risks", []):
            risk_level = risk.get("level", "medio")
            ...
            if risk_level == "alto":
                recommendations.append({
                    ...
                    "title": f"🔴 RIESGO ALTO DETECTADO: {risk_type}",
                    "action": f"Revisar inmediatamente: {description}. Impacto: {impact}",
                    "assign_to": "Equipo de Compliance",
                    "deadline": "24 horas"
                })
                ...
                recommendations.append({
                    ...
                    "title": f"🟡 RIESGO MEDIO: {risk_type}",
                    "action": f"Evaluar y mitigar: {description}",
                    "assign_to": "Equipo de Riesgos",
                    "deadline": "7 días"
                })
        
        # 3. RECOMENDACIONES DE COMPLIANCE
        # Detectar compliance basado en contenido
        all_text = json.dumps(extracted_data).lower()
        if "gdpr" in all_text or "datos personales" in all_text:
            recommendations.append({
                "type": "compliance",
                "priority": "alta",
                "category": "gdpr",
                "title": "🔒 VERIFICACIÓN GDPR REQUERIDA",
                "action": "Verificar cumplimiento de GDPR. Revisar manejo de datos personales.",
                "assign_to": "Equipo de Compliance",
                "deadline": "7 días"
            })
        
        if "pci" in all_text or "tarjeta" in all_text or "pago" in all_text:
            recommendations.append({
                "type": "compliance",
                "priority": "alta",
                "category": "pci",
                "action": "Verificar cumplimiento PCI-DSS para datos de tarjetas de crédito.",
                "assign_to": "Equipo de Seguridad",
                "deadline": "7 días"
            })
        
        # 4. RECOMENDACIONES DE WORKFLOW
        # Si hay obligaciones sin responsable asignado
        for obligation in extracted_data.get("obligations", []):
            if not obligation.get("responsible"):
                recommendations.append({
                    "type": "workflow",
                    "priority": "media",
                    "category": "asignacion_pendiente",
                    "title": "👤 ASIGNAR RESPONSABLE",
                    "action": f"Asignar responsable para: {obligation.get('description', 'obligación sin descripción')}",
                    "assign_to": "Gerente de Proyecto",
                    "deadline": "3 días"
                })
        
        # 5. RECOMENDACIONES DE DUPLICADOS (si se detecta)
        if "duplicado" in file_name.lower() or "copy" in file_name.lower():
            recommendations.append({
                "type": "workflow",
                "priority": "baja",
                "category": "archivo_duplicado",
                "title": "📋 ARCHIVO DUPLICADO DETECTADO",
                "action": "Verificar si es necesario mantener ambas versiones o archivar duplicado.",
                "assign_to": "Administrador de Documentos",
                "deadline": "14 días"
            })
    ...
    async def stream_realtime_insights(self, session_id: str) -> AsyncIterator[Dict[str, Any]]:
        ...
        Usage:
            async for insight in extraction_x_mode.stream_realtime_insights(session_id):
                print(insight)
        """
        last_index = 0
    ...
    async def _stream_query_insights(
        ...
        try:
            ...
            print(f"⚠️ [Extraction X Mode] Error en streaming de insights: {e}")
    ...
    async def process_query_async(
        ...
        try:
            ...
            print(f"⚠️ [Extraction X Mode] Error agregando pasos de razonamiento: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Extraction X Mode] Error en Reinforcement Planning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Extraction X Mode] Error en Path-dependent Reasoning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [Extraction X Mode] Error consultando MCP: {e}")
        ...
        try:
            ...
            formatted_answer += "- 📡 **Extraction X:** Arquitectura event-driven para procesamiento automático\n"
        ...
        try:
            ...
            for conn_id, connection in self.mcp_manager.connections.items():
                ...
                try:
                    ...
                    print(f"⚠️ [Extraction X Mode] Error consultando MCP {connection.name}: {e}")
                    continue
            ...
            print(f"⚠️ [Extraction X Mode] Error en consulta MCP: {e}")
            return None
    ...
    def handle_webhook_event(self, event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"📡 [Extraction X] Webhook recibido: {event_type}")
            ...
            print(f"❌ [Extraction X] Error procesando webhook: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    def sync_document_source(self, source: str, interval: int = 60) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"🔄 [Extraction X] Sincronizando desde {source} (intervalo: {interval}s)")
            ...
            print(f"❌ [Extraction X] Error en sync: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    def subscribe_to_external_webhook(self, source: str, callback_url: str) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"🔔 [Extraction X] Suscribiendo a webhooks de {source} -> {callback_url}")
            ...
            print(f"❌ [Extraction X] Error suscribiendo webhook: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    async def _process_query_parallel(
        ...
        Mantiene toda la funcionalidad específica de Extraction X Mode (extracción avanzada, recomendaciones accionables).
        """
        ...
        def analyze_single_document(source_name: str, file_docs: List[Document]) -> Tuple[str, str]:
            """Analiza un solo documento con el prompt del usuario (Extraction X Mode)."""
            try:
                doc_content = "\n\n".join([doc.page_content for doc in file_docs])
                if len(doc_content) > 4000:
                    doc_content = doc_content[:4000] + "..."
                
                # Prompt adaptado para Extraction X Mode (extracción avanzada y recomendaciones accionables)
                prompt = f"""Eres un analista estratégico de nivel C-Suite especializado en EXTRACCIÓN INTELIGENTE de documentos enterprise. Tu tarea es analizar ESTE documento específico para responder DIRECTAMENTE la pregunta del usuario, usando el formato específico de Extraction X Mode.
...
INSTRUCCIONES CRÍTICAS PARA EXTRACTION X MODE:
...
2. EXTRACCIÓN INTELIGENTE (formato específico de Extraction X Mode):
   - 🔍 **Entidades Extraídas:** Identifica personas, empresas, organizaciones, productos, servicios mencionados
   - 📅 **Fechas Críticas:** Detecta vencimientos, plazos, expiraciones con cálculo de días restantes si aplica
   - ⚠️ **Riesgos Identificados:** Clasifica riesgos como alto/medio/bajo (legales, financieros, operacionales, compliance)
   - 📋 **Obligaciones:** Identifica contratos, regulaciones, compromisos, requisitos
   - 📊 **Métricas y Valores:** Extrae números importantes, porcentajes, montos, cantidades
   - 👥 **Roles y Responsabilidades:** Identifica quién es responsable de qué

3. RECOMENDACIONES ACCIONABLES (formato específico de Extraction X Mode):
   - 🚨 **Recomendaciones Operativas:** Acciones inmediatas (renovaciones, vencimientos, asignaciones)
   - 🔴 **Recomendaciones de Riesgo:** Acciones para mitigar riesgos altos/medios detectados
   - 🔒 **Recomendaciones de Compliance:** Verificaciones GDPR, PCI-DSS, etc. si aplica
   - 🔄 **Recomendaciones de Workflow:** Asignaciones, notificaciones, flujos automáticos sugeridos
...
4. RESPUESTA ESTRUCTURADA (300-500 palabras):
   ...
   - **Extracción Inteligente** (1-2 párrafos): Presenta entidades, fechas, riesgos, obligaciones extraídas
   - **Recomendaciones Accionables** (1 párrafo): Recomendaciones específicas y accionables basadas en la extracción

5. FORMATO ESPECÍFICO DE EXTRACTION X MODE:
   - Usa emojis para categorizar: 🔍 Entidades, 📅 Fechas, ⚠️ Riesgos, 📋 Obligaciones, 🚨 Recomendaciones
   - Clasifica riesgos: 🔴 Alto, 🟡 Medio, 🟢 Bajo
   - Proporciona recomendaciones con acciones concretas, no solo descripciones

RESPUESTA ESPECÍFICA A LA PREGUNTA DEL USUARIO CON EXTRACCIÓN INTELIGENTE Y RECOMENDACIONES ACCIONABLES (300-500 palabras):"""
        ...
        print(f"🔄 [Extraction X Mode] Procesando {len(docs_by_source)} documentos en paralelo...")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"✅ [Extraction X Mode] Análisis completado para: {Path(doc_name).name}")
                    ...
                    print(f"❌ [Extraction X Mode] Error procesando {source_name}: {e}")
        ...
        synthesis_prompt = f"""Eres un consultor estratégico senior de nivel C-Suite especializado en EXTRACCIÓN INTELIGENTE de documentos enterprise y generación de recomendaciones accionables. Has analizado {len(individual_analyses)} documentos individualmente, cada uno respondiendo la pregunta del usuario usando el formato específico de Extraction X Mode.

TU TAREA PRINCIPAL: Combinar todos los análisis individuales para responder DIRECTAMENTE la pregunta del usuario de manera completa y estratégica, manteniendo el formato específico de Extraction X Mode.

PREGUNTA ESPECÍFICA DEL USUARIO (RESPONDE EXACTAMENTE ESTO):
{message}

ANÁLISIS INDIVIDUALES DE CADA DOCUMENTO (cada uno ya respondió la pregunta del usuario con extracción inteligente):
{combined_context}

INSTRUCCIONES PARA RESPUESTA FINAL COMBINADA (800-1200 palabras) - FORMATO EXTRACTION X MODE:
...
2. SÍNTESIS DE EXTRACCIÓN INTELIGENTE (formato Extraction X Mode):
   - 🔍 **Entidades Consolidadas:** Agrupa y consolida entidades de todos los documentos
   - 📅 **Fechas Críticas Consolidadas:** Identifica las fechas más críticas de todos los documentos
   - ⚠️ **Riesgos Consolidados:** Agrupa riesgos por nivel (alto/medio/bajo) y tipo
   - 📋 **Obligaciones Consolidadas:** Consolida obligaciones de todos los documentos
   - 📊 **Métricas Agregadas:** Combina métricas y valores numéricos de todos los documentos

3. RECOMENDACIONES ACCIONABLES CONSOLIDADAS (formato Extraction X Mode):
   - 🚨 **Recomendaciones Operativas Prioritarias:** Las acciones más urgentes de todos los documentos
   - 🔴 **Recomendaciones de Riesgo Prioritarias:** Acciones para mitigar los riesgos más altos
   - 🔒 **Recomendaciones de Compliance:** Verificaciones necesarias consolidadas
   - 🔄 **Recomendaciones de Workflow:** Flujos automáticos sugeridos basados en todos los documentos
...
5. FORMATO ESPECÍFICO DE EXTRACTION X MODE:
   - Usa emojis para categorizar: 🔍 Entidades, 📅 Fechas, ⚠️ Riesgos, 📋 Obligaciones, 🚨 Recomendaciones
   - Clasifica riesgos: 🔴 Alto, 🟡 Medio, 🟢 Bajo
   - Proporciona recomendaciones con acciones concretas y priorización

RESPUESTA FINAL QUE RESPONDE DIRECTAMENTE LA PREGUNTA DEL USUARIO CON EXTRACCIÓN INTELIGENTE Y RECOMENDACIONES ACCIONABLES CONSOLIDADAS (800-1200 palabras):"""
    ...
    def get_extractions(self, session_id: str) -> List[Dict[str, Any]]:
        """Obtiene todas las extracciones inteligentes de una sesión."""
        if session_id not in self.sessions:
            return []
        return self.sessions[session_id].get("extractions", [])
    
    def get_recommendations(self, session_id: str) -> List[Dict[str, Any]]:
        """Obtiene todas las recomendaciones accionables de una sesión."""
        if session_id not in self.sessions:
            return []
        
        all_recommendations = []
        for extraction in self.sessions[session_id].get("extractions", []):
            recommendations = extraction.get("recommendations", [])
            for rec in recommendations:
                rec["source_file"] = extraction.get("file_name", "unknown")
                all_recommendations.append(rec)
        
        # Ordenar por prioridad (alta -> media -> baja)
        priority_order = {"alta": 0, "media": 1, "baja": 2}
        all_recommendations.sort(key=lambda x: priority_order.get(x.get("priority", "baja"), 2))
        
        return all_recommendations
    
    def get_actionable_summary(self, session_id: str) -> Dict[str, Any]:
        """Genera un resumen accionable de todas las extracciones."""
        if session_id not in self.sessions:
            return {}
        
        extractions = self.sessions[session_id].get("extractions", [])
        recommendations = self.get_recommendations(session_id)
        
        # Agregar estadísticas
        total_entities = sum(e.get("summary", {}).get("total_entities", 0) for e in extractions)
        total_dates = sum(e.get("summary", {}).get("total_dates", 0) for e in extractions)
        urgent_dates = sum(e.get("summary", {}).get("urgent_dates", 0) for e in extractions)
        expired_dates = sum(e.get("summary", {}).get("expired_dates", 0) for e in extractions)
        total_risks = sum(e.get("summary", {}).get("total_risks", 0) for e in extractions)
        high_risks = sum(e.get("summary", {}).get("high_risk_count", 0) for e in extractions)
        total_obligations = sum(e.get("summary", {}).get("total_obligations", 0) for e in extractions)
        ...
        return {
            ...
            "total_critical_dates": total_dates,
...
# Instancia global
_extraction_x_mode_instance: Optional[ExtractionXMode] = None


def get_extraction_x_mode(
    config: AppConfig,
    processor: DocumentProcessor,
    retriever_builder: RetrieverBuilder,
    context_manager: Optional[Any] = None
) -> ExtractionXMode:
    """Obtiene o crea la instancia global de Extraction X Mode."""
    global _extraction_x_mode_instance
    
    if _extraction_x_mode_instance is None:
        _extraction_x_mode_instance = ExtractionXMode(
            config=config,
            processor=processor,
    ...
    return _extraction_x_mode_instance


def run_extraction_x_mode(
    message: str,
    history: List[Tuple[str, str]],
    files: List[Any],
    session_id: str,
    ...
    """
    Función principal para ejecutar Extraction X Mode.
    ...
    # Obtener instancia
    extraction_x_mode = get_extraction_x_mode(
        config=config,
        processor=processor,
        retriever_builder=retriever_builder,
        context_manager=context_manager
    ...
    if files:
        result = extraction_x_mode.process_documents(session_id, files)
    ...
    try:
        ...
        new_history, error, metadata = loop.run_until_complete(
            extraction_x_mode.process_query_async(
                session_id=session_id,
                message=message,
                history=history,
                speed_mode=speed_mode,
