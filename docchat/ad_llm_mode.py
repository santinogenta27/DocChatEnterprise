"""
AD LLM - Sistema Multi-Agente RAG de Máxima Calidad
...
class ADLLMMode:
    """
    AD LLM - Sistema Multi-Agente RAG de Máxima Calidad para Empresas.
    ...
    def __init__(
        ...
        if not config.openai_api_key:
            raise ValueError("OPENAI_API_KEY requerida para AD LLM")
        ...
        # LLM Generated Ads System (Papers Implementation)
        self.llm_generated_ads = LLMGeneratedAdsSystem(config=config, llm=self.llm)
    ...
    def process_documents(
        ...
        try:
            print(f"📄 [AD LLM] Procesando {len(new_files)} nuevos documentos...")
            ...
            if session["docs"]:
                ...
                print(f"✅ [AD LLM] Retriever actualizado: {len(session['docs'])} chunks")
            ...
            print(f"❌ [AD LLM] Error procesando documentos: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    ...
    async def process_query_async(
        ...
        try:
            ...
            print(f"⚠️ [AD LLM] Error agregando pasos de razonamiento: {e}")
        ...
        try:
            ...
            print(f"⚠️ [AD LLM] Error en Reinforcement Planning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [AD LLM] Error en Path-dependent Reasoning: {e}")
        ...
        try:
            ...
            print(f"⚠️ [AD LLM] Error consultando MCP: {e}")
        ...
        try:
            ...
            for conn_id, connection in self.mcp_manager.connections.items():
                ...
                try:
                    ...
                    print(f"⚠️ [AD LLM] Error consultando MCP {connection.name}: {e}")
                    continue
            ...
            print(f"⚠️ [AD LLM] Error en consulta MCP: {e}")
            return None
...
RESPUESTA ESPECÍFICA A LA PREGUNTA DEL USUARIO (300-500 palabras):"""
        ...
        print(f"🔄 [AD LLM] Procesando {len(docs_by_source)} documentos en paralelo...")
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    print(f"✅ [AD LLM] Análisis completado para: {Path(doc_name).name}")
                except Exception as e:
                    print(f"❌ [AD LLM] Error procesando {source_name}: {e}")
...
RESPUESTA FINAL QUE RESPONDE DIRECTAMENTE LA PREGUNTA DEL USUARIO (800-1200 palabras):"""
    ...
    # ============================================
    # LLM GENERATED ADS METHODS (Papers Implementation)
    # ============================================
    
    async def generate_personality_ad(
        self,
        product_name: str,
        personality_trait: str,
        product_description: Optional[str] = None
    ) -> Dict[str, Any]:
        """Genera anuncio personalizado basado en personalidad."""
        trait_enum = PersonalityTrait(personality_trait)
        variation = await self.llm_generated_ads.generate_personality_targeted_ad(
            product_name=product_name,
            personality_trait=trait_enum,
            product_description=product_description
        )
        return {
            "variation_id": variation.variation_id,
            "ad_text": variation.ad_text,
            "headline": variation.headline,
            "description": variation.description,
            "personality_target": variation.personality_target.value if variation.personality_target else None,
            "confidence_score": variation.confidence_score
        }
    
    async def generate_persuasion_ad(
        self,
        product_name: str,
        principle: str,
        product_description: Optional[str] = None
    ) -> Dict[str, Any]:
        """Genera anuncio basado en principio de persuasión."""
        principle_enum = PersuasionPrinciple(principle)
        variation = await self.llm_generated_ads.generate_persuasion_principle_ad(
            product_name=product_name,
        ...
        return {
            ...
            "headline": variation.headline,
            "description": variation.description,
            "persuasion_principle": variation.persuasion_principle.value if variation.persuasion_principle else None,
            "confidence_score": variation.confidence_score
        }
    
    async def predict_ad_ctr(
        self,
        ad_text: str,
        headline: str,
        description: str
    ) -> Dict[str, Any]:
        """Predice CTR con explicabilidad (SODA framework)."""
        prediction = await self.llm_generated_ads.predict_ctr_with_explanation(
            ad_text=ad_text,
            headline=headline,
        ...
        return {
            ...
            "ctr_percentage": prediction.predicted_ctr * 100,
            "confidence": prediction.confidence,
            "category": prediction.category,
            "key_factors": prediction.key_factors,
            "improvement_suggestions": prediction.improvement_suggestions,
            "attention_heatmap": prediction.attention_heatmap
        }
    ...
    async def analyze_ad(
        ...
        """Analiza un anuncio usando LLMs (SODA framework)."""
        analysis = await self.llm_generated_ads.analyze_ad_content(
            ad_text=ad_text,
            headline=headline,
            description=description
        )
        return {
            ...
            "human_need": analysis.human_need,
            "product_category": analysis.product_category,
            "archetype": analysis.archetype,
            "emotional_appeal": analysis.emotional_appeal,
            "stylistic_tone": analysis.stylistic_tone,
            "content_pillars": analysis.content_pillars,
            "target_persona": analysis.target_persona,
            "persuasion_principle": analysis.persuasion_principle.value if analysis.persuasion_principle else None
        }
    
    async def generate_optimized_variations(
        self,
        original_ad_text: str,
        product_name: str,
        num_variations: int = 5
    ) -> List[Dict[str, Any]]:
        """Genera variaciones optimizadas usando RLPF."""
        variations = await self.llm_generated_ads.generate_optimized_ad_variations(
            original_ad_text=original_ad_text,
            product_name=product_name,
        ...
        return [
            {
                ...
                "ctr_percentage": v.predicted_ctr * 100,
    ...
    async def compare_ads(
        ...
        """Compara anuncio AI vs humano."""
        # Crear AdVariation para AI ad
        ai_variation = AdVariation(
            variation_id=f"AI-{uuid.uuid4().hex[:8].upper()}",
            ad_text=ai_ad_text,
            headline=ai_headline,
            description=ai_description
        )
        
        comparison = await self.llm_generated_ads.compare_ai_vs_human_ads(
            ai_ad=ai_variation,
            human_ad_text=human_ad_text,
...
# Instancia global
_ad_llm_mode_instance: Optional[ADLLMMode] = None
...
def get_ad_llm_mode(
    ...
    """Obtiene o crea la instancia global de AD LLM."""
    global _ad_llm_mode_instance
    
    if _ad_llm_mode_instance is None:
        _ad_llm_mode_instance = ADLLMMode(
            config=config,
            processor=processor,
...
def run_ad_llm_mode(
    ...
    """
    Función principal para ejecutar AD LLM.
    ...
    if files:
        result = ad_llm_mode.process_documents(session_id, files)
    ...
    try:
        ...
        new_history, error, metadata = loop.run_until_complete(
            ad_llm_mode.process_query_async(
                session_id=session_id,
                message=message,
                history=history,
                speed_mode=speed_mode,
